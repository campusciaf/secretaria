<?php
    require_once("../modelos/PagosRematriculaAgregador.php");
    require_once("../YeminusApi/modelos/Yeminus.php");
    $obj_pago = new PagosRematriculaAgregador();
    $yeminus = new Yeminus();
	...
	$tiempo_pago = isset($_GET["x_extra4"])?$_GET["x_extra4"]:"";// tipo matricula del estudiante pp-e extra-e u ordinaria-e
    $matricula = isset($_GET["x_extra5"])?$_GET["x_extra5"]:"";// trae el datos si es matricula completa 1 o si es media matricula 2
    $semestre = isset($_GET["x_extra6"])?$_GET["x_extra6"]:""; // trae el dato de semestre 
    //Datos para Yemminus
    $forma_pago_yeminus = isset($_GET["x_extra7"]) ? $_GET["x_extra7"] : ""; // Forma de pago para yeminus
    $cuenta_contable_yeminus = isset($_GET["x_extra8"]) ? $_GET["x_extra8"] : ""; // cuenta contable para yeminus
    $realiza_proceso = isset($_GET["x_extra9"]) ? $_GET["x_extra9"] : ""; //define por que metodo se hizo, si estudiante, ticket o administrativa
    $codigo_producto = array("pp-e" => "MA01", "ordinaria-e" => "MA01", "extra-e" => "MA01"); //dependiendo del tiempo en que paga es el codigo
    //datos Epayco 
    ...
    if($stmt){
        echo json_encode(array("status" => "OK<br>"));
    }else{
        echo json_encode(array("status" => "ERR<br>"));
    }
