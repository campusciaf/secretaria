...
function init() {
	...
	$.post("../controlador/web_blog.php?op=Categoria_noticia", function (r) {
		$("#categoria_blog").html(r);
		$("#categoria_blog").selectpicker("refresh");
	});

	$.post("../controlador/web_blog.php?op=Categoria_video", function (r) {
...
function listarnoticias() {
	...
	tabla = $("#tbllistanoticias").dataTable({
		...
		ajax: {
			url: "../controlador/web_blog.php?op=listarnoticias",
			type: "get",
			dataType: "json",
			error: function (e) { },
		},
...
function limpiar_imagen() {
	...
	$("#contenido_blog_imagen").val("");
	$("#mostrar_video_noticia").attr("src", " ");
	$("#guardaimagennoticia").val("");
	$("#categoria_blog_imagen").val("");
	$("#agregar_imagen").val("");
...
function mostrar_blog(id_blog) {
	...
	$.post(
		"../controlador/web_blog.php?op=mostrar_blog",
		{ id_blog: id_blog },
		function (data) {
			...
			if (Object.keys(data).length > 0) {
				...
				$("#categoria_blog").val(data.id_categoria_blog);
				$("#categoria_blog").selectpicker("refresh");
...
function mostrar_video(id_blog) {
	$.post(
		"../controlador/web_blog.php?op=mostrar_video",
		{ id_blog: id_blog },
		function (data) {
			data = JSON.parse(data);
			if (Object.keys(data).length > 0) {
				...
				$("#categoria_blog_video").val(data.id_categoria_blog);
				$("#categoria_blog_video").selectpicker("refresh");
...
function mostrar_imagen(id_blog) {
	$("#ModalVideo").modal("show");
	$.post(
		"../controlador/web_blog.php?op=mostrar_imagen",
		{ id_blog: id_blog },
		function (data) {
			data = JSON.parse(data);
			if (Object.keys(data).length > 0) {
				...
				$("#categoria_blog_imagen").val(data.id_categoria_blog);
				$("#categoria_blog_imagen").selectpicker("refresh");
...
function agregarvideo(e) {
	...
	$.ajax({
		url: "../controlador/web_blog.php?op=agregarvideo",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function agregarimagen(e) {
	...
	$.ajax({
		url: "../controlador/web_blog.php?op=agregarimagen",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function guardaryeditarnoticias(e) {
	...
	$.ajax({
		url: "../controlador/web_blog.php?op=guardaryeditarnoticias",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
	...
	swalWithBootstrapButtons
		...
		.then((result) => {
			if (result.isConfirmed) {
				$.post(
					"../controlador/web_blog.php?op=eliminar_blog",
...
function desactivar_blog(id_blog) {
	const swalWithBootstrapButtons = Swal.mixin({
		customClass: {
			confirmButton: "btn btn-success",
			cancelButton: "btn btn-danger",
	...
	swalWithBootstrapButtons
		.fire({
			title: "¿Está Seguro de desactivar el blog?",
			text: "¡No podrás revertir esto!",
			icon: "warning",
			showCancelButton: true,
			confirmButtonText: "Yes, continuar!",
		...
		.then((result) => {
			if (result.isConfirmed) {
				$.post(
					"../controlador/web_blog.php?op=desactivar_noticia",
					{ id_blog: id_blog },
					function (e) {
						if (e == "null") {
							swalWithBootstrapButtons.fire({
								title: "Ejecutado!",
								text: "Blog desactivado con éxito.",
								icon: "success",
							});
...
function activar_noticia(id_blog) {
	...
	swalWithBootstrapButtons
		...
		.then((result) => {
			if (result.isConfirmed) {
				$.post(
					"../controlador/web_blog.php?op=activar_noticia",
					{ id_blog: id_blog },
					function (e) {
						if (e == "null") {
							swalWithBootstrapButtons.fire({
								title: "Ejecutado!",
								text: "Blog activado con éxito.",
								icon: "success",
							});
...
function imagenyvideo(id_blog) {
	$.post(
		"../controlador/web_blog.php?op=imagenyvideo",
		{ id_blog: id_blog },
		function (data) {
			data = JSON.parse(data);
...
function ver_video(id_blog) {
	$.post(
		"../controlador/web_blog.php?op=ver_video",
		{ id_blog: id_blog },
		function (data) {
			data = JSON.parse(data);
