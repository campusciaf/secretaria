...
function mostrarLibros(){
	$.ajax({
		url:'../controlador/bibliotecaestudiantes.php?op=mostrar',
...
function BusquedaLibros(busqueda){
	$.ajax({
		type:'POST',
		url:'../controlador/bibliotecaestudiantes.php?op=buscar',
...
function abrirModalLibro(id_libro){
	...
	$.ajax({
		type:'POST',
		url:'../controlador/bibliotecaestudiantes.php?op=cargarFormulario',
		data:{id_libro:id_libro},
		success:function(msg){
			datos = JSON.parse(msg);
			$('#tituloModalBiblioteca').html(datos[0][2]);
			$("#id_libro").val(datos[0][0]);
			$("#imagen_editar").html('<img src="../public/img/bibliotecaestudiantes/'+datos[0][1]+'" width="35%"><br><label>Imagen Actual</label><input name="respaldoimagen" type="text" class="hidden" value="'+datos[0][1]+'">');
...
function  EnviarInformacion(accion, objEvento){
	$.ajax({
		type:'POST',
		url:'../controlador/bibliotecaestudiantes.php?op='+accion,
		data:formData,
		contentType: false,
        processData: false,
		success:function(msg){
