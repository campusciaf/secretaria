...
function init() {
    ...
    $(".filtros").hide();
    ListarRango();
    $('.yeminus_tooltip').tooltip();
    //Cargamos los items de los selects
    $.post("../controlador/web_transacciones.php?op=selectPeriodo", function (r) {
...
function listar(tipo_busqueda, dato_busqueda) {
    ...
    tabla = $('#tbllistado').DataTable({
        ...
        "buttons": [{
            ...
            "text": 'Filtrar Pago',
            "buttons": [
                { "text": 'Todos', action: function () { tabla.columns(8).search("").draw() } },
                { "text": 'Efectivo (Epayco)', action: function () { tabla.columns(8).search("EF|DP", true, false).draw() } },
                { "text": 'PSE (Occidente)', action: function () { tabla.columns(8).search("PSE|MC|VS|AM|DC", true, false).draw() } },
            ],
            "autoClose": true,
        }, 
    ],
        "ajax": {
            "url": '../controlador/web_transacciones.php?op=listar',
            "type": "POST",
...
function detalletransaccion(id_pago) {
    $.post("../controlador/web_transacciones.php?op=detalletransaccion", { id_pago: id_pago }, function (data, status) {
