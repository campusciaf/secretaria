...
function cargarCategoriasSuperiores() { 
    $.post("../controlador/puntos.php?op=totales_generales", {
...
function cargarCategorias() {
    $.post("../controlador/puntos.php?op=listar_categorias", {
        fecha_inicio: formatFecha(fechaInicio),
        fecha_fin: formatFecha(fechaFin)
    }, function (data) {
...
function graficoPuntos(categoria) { 
    ...
    $.post("../controlador/puntos.php?op=grafico_puntos", {
        categoria,
        fecha_inicio: formatFecha(fechaInicio),
        fecha_fin: formatFecha(fechaFin)
    }, function (data) {
...
function cargarListadoCategoria(categoria) {
    $.post("../controlador/puntos.php?op=listado_personas_categoria", {
        categoria: categoria,
        fecha_inicio: formatFecha(fechaInicio),
        fecha_fin: formatFecha(fechaFin)
    }, function (data) {
        ...
        if (personas.length === 0) {
            ...
            personas.forEach(p => {
                const nombreCompleto = `${p.credencial_nombre} ${p.credencial_nombre_2 ?? ''} ${p.credencial_apellido} ${p.credencial_apellido_2 ?? ''}`;
                tbody.append(`
                    <tr>
                        <td>${p.credencial_identificacion}</td>
...
function puntos_totales() {
    $.post("../controlador/puntos.php?op=puntos_totales", {
        fecha_inicio: formatFecha(fechaInicio),
        fecha_fin: formatFecha(fechaFin)
    }, function (e) {
