...
function  cargarperiodo(){
  ...
  $.post("../controlador/factordocente.php?op=selectPeriodo", function(r){
    $("#periodo").html(r);
...
function cargarjornadas(){
  $.post("../controlador/factordocente.php?op=selectJornada", function(r){
      $("#jornada").html(r);
...
function cargarprograma(escuela,nivel){
  $.post("../controlador/factordocente.php?op=selectPrograma",{escuela:escuela, nivel:nivel},function(r){
      $("#programa_ac").html(r);
...
//Cargamos los items de los selects contrato
function nivelformacion(){
  $.post("../controlador/factordocente.php?op=selectNivelFormacion",{},function(r){
...
function buscar(){
    ...
    select5 = document.getElementById('contrato');
    for (var i = 0; i < select5.options.length; i++) {
      o = select5.options[i];
      if (o.selected == true) {
        contrato.push(o.value);
      }
    }
    ...
    select6 = document.getElementById('nivel_formacion');
    for (var i = 0; i < select6.options.length; i++) {
      o = select6.options[i];
      if (o.selected == true) {
        nivel_formacion.push(o.value);
      }
    }

    if(nivel_formacion.length == 0){
      nivel_formacion="0";
    }else{
      nivel_formacion;
    }

    console.log(nivel_formacion);
    $.post("../controlador/factordocente.php?op=general",{periodo:periodo, escuela:escuela, nivel:nivel, jornada:jornada, semestre:semestre, programa:programa, contrato:contrato, nivel_formacion:nivel_formacion}, function(r){
