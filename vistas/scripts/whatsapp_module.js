...
$(document).ready(whatsapp_module_init);
...
function whatsapp_module_init() {
    verificarCambioNumeroSeleccionado(); // Listar los chats disponibles
    setInterval(verificarCambioNumeroSeleccionado, 10000); // Refrescar la lista de chats cada 10 segundos
    $(".recording-text").hide();
...
function listarDatos(numero_celular) {
    numero_seleccionado = numero_celular
    // Cargar los datos del chat desde el servidor
    $.post("../controlador/whatsapp_module.php?op=listarConversacion", { "numero_celular": numero_celular }, function (r) {
        //console.log(r)
        ...
        if (r.exito == 1) {
            ...
            if (!mismo_numero || (mismo_numero && total_sin_mostrar >= 1 || envio_mensaje)) {
                if (total_sin_mostrar >= 1) {
                    activarNotificaciones();
                }
                $('.btn_agregar_tarea').attr('onclick', 'listarDatosTareas(' + r.id_credencial + ')');
                ...
                $("#info_fecha_nacimiento").text(r.fecha_nacimiento); // Mostrar la fecha de nacimiento  
                $("#info_tipo_documento").text(r.tipo_documento); // Mostrar la fecha de nacimiento    
                $("#info_direccion").text(r.direccion); // Mostrar la fecha de nacimiento 
                $("#info_email").text(r.email); // Mostrar la fecha de nacimiento 
                $("#info_email_ciaf").text(r.email_ciaf); // Mostrar la fecha de nacimiento    
...
function sendWhatsappMessage() {
    ...
    $.ajax({
        "url": "../controlador/whatsapp_module.php?op=sendWhatsappMessage",
        "type": "POST",
        "data": formData,
        "contentType": false,
        "processData": false,
        success: function (datos) {
            ...
            if (data.exito == "1") {
                $("#variables_template").val(""); // Limpia la cantidad de variables de la plantilla
...
function listarTemplates() {
    ...
    $("#templateList").dataTable({
        ...
        "ajax": {
            "url": '../controlador/whatsapp_module.php?op=listarTemplates',
            "type": "post",
            "dataType": "json",
            "error": function (e) {
        ...
        "bDestroy": true,
        "iDisplayLength": 15, // Paginación: cantidad de plantillas mostradas por página
    }).DataTable();
}
// Función para seleccionar una plantilla y enviar el mensaje correspondiente
function seleccionarTemplate(nombre_template, variables_template) {
    ...
    $("#variables_template").val(variables_template); // Obtener la cantidad de variables de la plantilla
...
function verificarSeguimientoActivo(numero_celular) {
    $.post("../controlador/whatsapp_module.php?op=verificarSeguimientoActivo", { "numero_celular": numero_celular }, function (r) {
        r = JSON.parse(r); // Parsear la respuesta JSON
        if (r.exito == 1) {
            ...
            $(".btn_redirigir_chat").attr("disabled", true); // Cuando esta activo el seguimiento no se puede regdigir
        } else if (r.exito == 2) {
            ...
            $(".btn_redirigir_chat").attr("disabled", false); // sino tiene seguimiento activo se puede regdigir
        } else {
            $("#formas_de_envio").hide(); // Ocultar las formas de envío
            ...
            $(".btn_redirigir_chat").attr("disabled", true); // Si esta acitvo por otro usuario no puede redirigir
        }
    });
}
...
function ActivarSeguimiento() {
    ...
    $.post("../controlador/whatsapp_module.php?op=ActivarSeguimiento", { "numero_celular": numero_celular }, function (r) {
        r = JSON.parse(r); // Parsear la respuesta JSON
        if (r.exito == 1) {
            $("#formas_de_envio").show(); // Mostrar las formas de envío
            $("#estado_seguimiento").hide(); // Ocultar el estado de seguimiento
            ...
            $(".btn_iniciar_seguimiento").attr("disabled", true); // Deshabilitar el botón de iniciar seguimiento
            $(".btn_redirigir_chat").attr("disabled", true); // Si esta acitvo no puede redirigir
        } else {
...
function FinalizarSeguimiento() {
    Swal.fire({
        ...
        if (resultado_seguimiento.isConfirmed) {
            numero_celular = $("#send_numero_celular").val(); // Obtener el número de celular del campo oculto
            $.post("../controlador/whatsapp_module.php?op=FinalizarSeguimiento", { "numero_celular": numero_celular, "resultado_seguimiento": resultado_seguimiento.value }, function (r) {
                r = JSON.parse(r); // Parsear la respuesta JSON
                if (r.exito == 1) {
                    $("#formas_de_envio").hide(); // Ocultar las formas de envío
                    $("#estado_seguimiento").show(); // Mostrar el estado de seguimiento
...
function ListarDependencias() {
    $.post("../controlador/whatsapp_module.php?op=ListarDependencias", {}, function (r) {
        r = JSON.parse(r); // Parsear la respuesta JSON
...
function redigirChat() {
    ...
    $("#tipo_mensaje").val("text");
    $.ajax({
        "url": "../controlador/whatsapp_module.php?op=redigirChat",
        "type": "POST",
...
function MarcarComoLeido() {
    numero_celular = $("#send_numero_celular").val(); // Obtener el número de celular del campo oculto
    $.post("../controlador/whatsapp_module.php?op=MarcarComoLeido", { "numero_celular": numero_celular }, function (r) {
        r = JSON.parse(r); // Parsear la respuesta JSON
        if (r.exito == 1) {
            // Mostrar repuesta con SweetAlert
            Swal.fire({ "position": "top-end", "icon": "success", "title": r.info });
        } else {
            // Mostrar error con SweetAlert
            Swal.fire({ "position": "top-end", "icon": "error", "title": r.info });
        }
    });
}
function listarDatosTareas(id_credencial) {
    $("#modalTareas").modal("show");
    verHistorial(id_credencial);
