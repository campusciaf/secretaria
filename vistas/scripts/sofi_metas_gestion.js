...
function listar(id_usuario) {
    ...
    tabla_atrasados = $('#tabla_atrasados').dataTable({
        ...
        "ajax": {
            "url": "../controlador/sofi_metas_gestion.php?op=listar",
            "type": "POST",
        ...
        "initComplete": function () {
            $("#precarga").hide();
            metaAsesor(id_usuario);
            metaAsesorPagos(id_usuario);
        }
    }).DataTable();
...
function verInfoSolicitante(id) {
    //muestra datos personales
    $.post("../controlador/sofi_metas_gestion.php?op=verInfoSolicitante", { id_persona: id }, function (data, status) {
    ...
    //muestra referencias
    $.post("../controlador/sofi_metas_gestion.php?op=verRefeSolicitante", { id_persona: id }, function (data) {
        data = JSON.parse(data);
        //console.log(data);
        var html = ``;
...
function verCuotas(consecutivo) {
    $('#tabla_cuotas').dataTable({
        ...
        "dom": 'rtip',
        "ajax": {
            "url": "../controlador/sofi_metas_gestion.php?op=verCuotas",
            "type": "POST",
...
function cambiarEtiqueta(id_persona,valor) {
	$.post("../controlador/sofi_metas_gestion.php?op=cambiarEtiqueta",{id_persona:id_persona, valor:valor},function(data){
		if (data == 1) {
...
function metaAsesor(id_usuario) {
	$.post("../controlador/sofi_metas_gestion.php?op=metasAsesor",{id_usuario:id_usuario},function(data){
...
function metaAsesorPagos(id_usuario) {
	$.post("../controlador/sofi_metas_gestion.php?op=metasAsesorPagos",{id_usuario:id_usuario},function(data){
        var r = JSON.parse(data);
...
function listar_colaboradores() {
    $.post("../controlador/sofi_metas_gestion.php?op=listarColaboradores", function (data, status) {
