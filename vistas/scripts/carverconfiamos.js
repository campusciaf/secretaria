...
function init() {
	...
	$.post("../controlador/carverconfiamos.php?op=selectPeriodo", function (r) {
		$("#periodo").html(r);
...
//Función Listar
function tablas() {
	$.post("../controlador/carverconfiamos.php?op=datostabla", {}, function (data, status) {
...
function listar(valor) {
	$("#precarga").show();
	$.post("../controlador/carverconfiamos.php?op=listar", {valor:valor }, function (data, status) {
		data = JSON.parse(data);
		$("#tbllistado").html("");
