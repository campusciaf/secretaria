...
function iniciarTour(){
	introJs().setOptions({
		...
		steps: [ 
			{
				title: 'Panel',
				intro: "Bienvenido a nuestro panel general donde encontrarás todas las categorias, y endrás acceso a toda la información correspondiente "
			},
...

			{
				...
				intro: "Aquí encontrarás el total de tareas de manera general  "
			},
			{
				title: 'Caso',
				element: document.querySelector('#t-co'),
				intro: "Encontrarás el número de caso"
			},
			{
				title: 'Campaña',
				element: document.querySelector('#t-cp'),
				intro: "Da un vistazo a nuestra campaña seleccionada en nuestro panel general"
			},
			{
				title: 'Estado',
				element: document.querySelector('#t-eta'),
				intro: "Aquí encontrarás el estado asignado en este caso lo visualizas como matriculado"
			},
			{
				title: 'Estado',
			...
			},
			{
				...
				intro: "Aquí encontrarás  todos los resultados interesadas en nuestra campañas anteriores "
			},
			{
...
function iniciarSegundoTour() {
	introJs().setOptions({
		...
		"steps": [{
			"title": 'Panel',
			"intro": 'Bienvenido a nuestro panel general de registros'
		},
		{
			"title": 'Programa',
			"element": document.querySelector('#t2-paso0'),
			"intro": "Aquí encontrarás los diferentes programas totales de interés para las personas "
		},
		{
			"title": 'Ninguno',
...
function listar(){
	$("#precarga").show();
	$.post("../controlador/oncenterpanel.php?op=listar",{},function(data, status){
		...
		$.post("../controlador/oncenterpanel.php?op=selectPeriodo",{},function(data, status){
    		data = JSON.parse(data);// convertir el mensaje a json
            var ancho=data["0"]["1"];
    		var a=0;
    		while(a < ancho){
...
function listarDos(periodo,estado){
	...
	$("#precarga").show();
	$.post("../controlador/oncenterpanel.php?op=listarDos",{periodo:periodo, estado:estado},function(data, status){	
		data = JSON.parse(data);// convertir el mensaje a json
...
function listarTres(nombre_estado,medio,periodo){
	...
	$.post("../controlador/oncenterpanel.php?op=listarTres",{nombre_estado:nombre_estado, medio:medio, periodo:periodo},function(data, status){	
		data = JSON.parse(data);// convertir el mensaje a json
...
var fecha_hoy=(diasSemana[f.getDay()] + ", " + f.getDate() + " de " + meses[f.getMonth()] + " de " + f.getFullYear());
	...
	tabla=$('#tbllistadoestudiantes').dataTable(
	{
		...
		"ajax":
				{
					url: '../controlador/oncenterpanel.php?op=verEstudiantes&nombre_programa='+nombre_programa+'&jornada='+jornada+'&estado='+estado+'&periodo='+periodo,
					type : "get",
					dataType : "json",						
					error: function(e){
...
var fecha_hoy=(diasSemana[f.getDay()] + ", " + f.getDate() + " de " + meses[f.getMonth()] + " de " + f.getFullYear());
	...
	tabla=$('#tbllistadoestudiantes').dataTable(
	{
		...
		"ajax":
				{
					url: '../controlador/oncenterpanel.php?op=verEstudiantesMedio&nombre_programa='+nombre_programa+'&jornada='+jornada+'&medio='+medio+'&estado='+estado+'&periodo='+periodo,
					type : "get",
					dataType : "json",						
					error: function(e){
						console.log(e.responseText);	
...
var fecha_hoy=(diasSemana[f.getDay()] + ", " + f.getDate() + " de " + meses[f.getMonth()] + " de " + f.getFullYear());
	...
	tabla=$('#tbllistadoestudiantes').dataTable(
	{
		...
		"ajax":
				{
					url: '../controlador/oncenterpanel.php?op=verEstudiantesSuma&jornada='+jornada+'&estado='+estado+'&periodo='+periodo,
					type : "get",
					dataType : "json",						
					error: function(e){
						console.log(e.responseText);	
...
var fecha_hoy=(diasSemana[f.getDay()] + ", " + f.getDate() + " de " + meses[f.getMonth()] + " de " + f.getFullYear());
	...
	tabla=$('#tbllistadoestudiantes').dataTable(
	{
		...
		"ajax":
				{
					url: '../controlador/oncenterpanel.php?op=verEstudiantesSumaMedio&jornada='+jornada+'&medio='+medio+'&estado='+estado+'&periodo='+periodo,
					type : "get",
					dataType : "json",						
					error: function(e){
						console.log(e.responseText);	
...
var fecha_hoy=(diasSemana[f.getDay()] + ", " + f.getDate() + " de " + meses[f.getMonth()] + " de " + f.getFullYear());
	...
	tabla=$('#tbllistadoestudiantes').dataTable(
	{
		...
		"ajax":
				{
					url: '../controlador/oncenterpanel.php?op=verEstudiantesTotal&estado='+estado+'&periodo='+periodo,
					type : "get",
					dataType : "json",						
					error: function(e){
						console.log(e.responseText);	
...
var fecha_hoy=(diasSemana[f.getDay()] + ", " + f.getDate() + " de " + meses[f.getMonth()] + " de " + f.getFullYear());
	...
	tabla=$('#tbllistadoestudiantes').dataTable(
	{
		...
		"ajax":
				{
					url: '../controlador/oncenterpanel.php?op=verEstudiantesTotalMedio&medio='+medio+'&estado='+estado+'&periodo='+periodo,
					type : "get",
					dataType : "json",						
					error: function(e){
						console.log(e.responseText);	
...
function selectEstado(){
	...
	$.post("../controlador/oncenterpanel.php?op=selectEstado", function(r){
		$("#estado").html(r);
...
function selectPeriodoDos(){
	//Cargamos los items de los selects
	$.post("../controlador/oncenterpanel.php?op=selectPeriodoDos", function(r){
		$("#periodo_dos").html(r);
...
function eliminar(id_estudiante)
{
	alertify.confirm('Eliminar Caso', '¿Está Seguro de eliminar el Caso?', function(){ 
		...
		$.post("../controlador/oncenterpanel.php?op=eliminar", {id_estudiante : id_estudiante}, function(e){
...
function guardarMoverUsuario(e4)
{
	...
	$.ajax({
		url: "../controlador/oncenterpanel.php?op=moverUsuario",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
...
function verHistorial(id_estudiante){
	$("#precarga").show();
	$.post("../controlador/oncenterpanel.php?op=verHistorial",{id_estudiante:id_estudiante},function(data, status){
...
var fecha_hoy=(diasSemana[f.getDay()] + ", " + f.getDate() + " de " + meses[f.getMonth()] + " de " + f.getFullYear());
	...
	tabla=$('#tbllistadohistorial').dataTable(
	{
		...
		"ajax":
				{
					url: '../controlador/oncenterpanel.php?op=verHistorialTabla&id_estudiante='+id_estudiante,
					type : "get",
					dataType : "json",						
					error: function(e){
						console.log(e.responseText);	
...
var fecha_hoy=(diasSemana[f.getDay()] + ", " + f.getDate() + " de " + meses[f.getMonth()] + " de " + f.getFullYear());
	...
	tabla=$('#tbllistadoHistorialTareas').dataTable(
	{
		...
		"ajax":
				{
					url: '../controlador/oncenterpanel.php?op=verHistorialTablaTareas&id_estudiante='+id_estudiante,
					type : "get",
					dataType : "json",						
					error: function(e){
						console.log(e.responseText);	
