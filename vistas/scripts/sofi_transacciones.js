...
function init() {
    ...
    $.post("../controlador/sofi_transacciones.php?op=selectPeriodo", function (r) {
...
function listar(tipo_busqueda, dato_busqueda) {
    ...
    tabla = $('#tbllistado').DataTable({
        ...
        "buttons": [{
            ...
            "text": 'Filtrar Pago',
            "buttons": [
                { "text": 'Todos', action: function () { tabla.columns(5).search("").draw() } },
                { "text": 'Efectivo (Epayco)', action: function () { tabla.columns(5).search("EF|DP", true, false).draw() } },
                { "text": 'PSE (Occidente)', action: function () { tabla.columns(5).search("PSE|MC|VS|AM|DC",true, false).draw() } },   
            ],
            "autoClose": true,
            },],
        "ajax": {
            "url": '../controlador/sofi_transacciones.php?op=listar',
            "type": "POST",
...
function detalletransaccion(id_pago) {
    $.post("../controlador/sofi_transacciones.php?op=detalletransaccion", { id_pago: id_pago }, function (data, status) {
