...
function agregarMensajeTodos() {
    ...
    $.ajax({
        "url": "../controlador/sofi_cuotas_a_vencer.php?op=agregarMensajeTodos",
        "type": "POST",
        "data": formData,
        "contentType": false,
        "processData": false,
    ...
    tabla_cuotas_a_vencer = $('#tabla_cuotas_a_vencer').dataTable({
        ...
        "buttons": [
            ...
            },
            {
                ...
                "action": function(){
					$.ajax({
						url:  "../controlador/sofi_cuotas_a_vencer.php?op=enviarMailTodos",
						type: "POST",
                        beforeSend: function() {
							$("#precarga").show();
						},
		...
		"ajax":{
			"url": "../controlador/sofi_cuotas_a_vencer.php?op=listarCuotasAVencer",
			"type" : "POST",
			"dataType" : "json",
			"error": function(e){
...
function enviarMail(id_persona){
    $("#precarga").show();
	$.post("../controlador/sofi_cuotas_a_vencer.php?op=enviarMail",{id_persona : id_persona}, function(data){
        $("#precarga").hide();
        //console.log(data);
		var datos = JSON.parse(data);	
		if(datos.exito == "1"){
...
function verInfoSolicitante(id){
    ...
    $.post("../controlador/sofi_cuotas_a_vencer.php?op=verInfoSolicitante",{id_persona : id}, function(data, status){
    ...
    //muestra referencias
    $.post("../controlador/sofi_cuotas_a_vencer.php?op=verRefeSolicitante",{id_persona : id},function(data){
        data = JSON.parse(data);
        //console.log(data);
        var html = ``;
...
function consutaTareaSeguimiento(id_persona, id_credencial) {
    $("#id_persona_seguimiento").val(id_persona);
...
function verCuotas(consecutivo){
    $('#tabla_cuotas').dataTable({
		...
		"dom": 'rtip',
		"ajax":{
			"url": "../controlador/sofi_cuotas_a_vencer.php?op=verCuotas",
			"type" : "POST",
...
function cambiarEtiqueta(id_persona,valor) {
	$.post("../controlador/sofi_cuotas_a_vencer.php?op=cambiarEtiqueta",{id_persona:id_persona, valor:valor},function(data){
		if (data == 1) {
