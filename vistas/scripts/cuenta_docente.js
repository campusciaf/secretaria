...
function listar(){
	...
	tabla = $('#tbllistado').dataTable({
		...
		"ajax":{
            "url": '../controlador/cuenta_docente.php?op=listar',
            "type" : "get",
            "dataType" : "json",						
             "error": function(e){
...
function listarGrupos(id_docente){
	...
	$("#informacion_grupos").show();
    $("#informacion_grupos").removeAttr("hidden");
	$("#boton_regresar").show();
...
$("#boton_regresar").off("click").on("click",function(){
    $("#listadoregistros").show();
    $("#informacion_grupos").attr("hidden",true);
	$("#boton_regresar").hide();
...
function generarSelectInput(id_docente){
	...
	$.ajax({
		type:'POST',
		url:'../controlador/cuenta_docente.php?op=cargarOpciones',
		...
		success:function(msg){
			...
			for (i = 0; i < posiciones; i++) {
				...
				option = '<option value="'+id_grupo+'" style="color:#000 !important">'+jornada+' - '+materia+' - '+programa+'</option>';
				$("#selector_grupo").append(option);
				
			}
			$('#selector_grupo').selectpicker('refresh');
...
$("#listado_grupos_docente").on("submit",function(e){
	...
	$.ajax({
		...
		"url" : '../controlador/cuenta_docente.php?op=listarEstudiantes',
		...
		"success":function(msg){
			...
			$("#estudiantes_grupo").dataTable({ 
				...
				"paging":   false,
				"searching": false,
				"iDisplayLength": false,//Paginación
...
$("#boton_ver_pea").off("click").on("click",function(e2){
	...
	$.ajax({
		"type" : 'POST',
		"url" : '../controlador/cuenta_docente.php?op=cargarDatosPrograma',
		"data" : {grupo_seleccionado:grupo_seleccionado},
		"success" : function(msg){
			var datos = JSON.parse(msg);
...
function comprobar(materia, id_programa, id_docente_grupo){
	$.ajax({
		"type" : 'POST',
		"url" : '../controlador/cuenta_docente.php?op=comprobar',
...
function listarPea(materia,id_programa,id_docente_grupo){
	...
	tabla = $('#tblpea').dataTable({
		...
		"ajax":{
            "url": '../controlador/cuenta_docente.php?op=listarPea&id_programa='+id_programa+'&materia='+materia+'&id_docente_grupo='+id_docente_grupo,
            "type" : "get",
            "dataType" : "json",						
            "error": function(e){
				console.log(e.responseText);	
...
function mostrarTemas(id_tema,id_docente_grupo){
	...
	$.post("../controlador/cuenta_docente.php?op=listaractividades",{id_tema:id_tema, id_docente_grupo:id_docente_grupo}, function(data){
...
function listarHorario(opcion,id_docente){
	$("#precarga").show();
	$.post("../controlador/cuenta_docente.php?op=listarHorario", {opcion:opcion,id_docente:id_docente}, function(data){
		data = JSON.parse(data);
		$("#tblhorarios").html(""); 
...
function iniciarcalendario(id_docente){
	...
	var calendar = new FullCalendar.Calendar(calendarEl, {
		...
		events:"../controlador/cuenta_docente.php?op=iniciarcalendario&id_docente="+id_docente,
