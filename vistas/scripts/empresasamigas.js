...
function init(){
	...
	$.post("../controlador/empresasamigas.php?op=selectPrograma", function(r){
	            $("#programa").html(r);
	            $('#programa').selectpicker('refresh');
	});
			//Cargamos los items de los selects contrato
			$.post("../controlador/empresasamigas.php?op=selectJornada", function(r){
	            $("#jornada").html(r);
	            $('#jornada').selectpicker('refresh');
	});
	
		//Cargamos los items de los selects contrato
	$.post("../controlador/empresasamigas.php?op=selectPeriodo", function(r){
	            $("#periodo").html(r);
...
$.post("../controlador/empresasamigas.php?op=datostabla",{}, function(data, status)
{
...
function comprobar()
{
	...
	if(id_programa!="" && periodo!=""){
		listar(id_programa,periodo);
	}

}


function listar(id_programa,periodo)
{
	
	$("#precarga").show();
	
	$.post("../controlador/empresasamigas.php?op=listar",{id_programa:id_programa, periodo:periodo}, function(data, status)
	{
		data = JSON.parse(data);
