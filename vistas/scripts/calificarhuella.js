...
function init() {
	...
	$.post("../controlador/calificarhuella.php?op=selectPrograma", function (r) {
		$("#fo_programa").html(r);
		$("#fo_programa").selectpicker("refresh");
	});

	$.post("../controlador/calificarhuella.php?op=selectJornada", function (r) {
		$("#jornada_e").html(r);
		$("#jornada_e").selectpicker("refresh");
	});

	$.post("../controlador/calificarhuella.php?op=selectPeriodo", function (r) {
		$("#periodo").html(r);
		$("#periodo").selectpicker("refresh");
	});

	$.post("../controlador/calificarhuella.php?op=selectGrupo", function (r) {
		$("#grupo").html(r);
...
function verificardocumento(e1) {
	...
	$.ajax({
		url: "../controlador/calificarhuella.php?op=verificardocumento",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function listar(id_credencial) {
	...
	tabla = $("#tbllistado")
		.dataTable({
			...
			ajax: {
				url:
					"../controlador/calificarhuella.php?op=listar&id_credencial=" +
					id_credencial,
				type: "get",
				dataType: "json",
				error: function (e) { }
...
function mostrardatos(id_credencial) {
	$.post(
		"../controlador/calificarhuella.php?op=mostrardatos",
		...
		function (data) {
			...
			if (data.exito == 1) {
				$(".box_nombre_estudiante").html(
					data.info.credencial_nombre +
					" " +
					data.info.credencial_nombre_2 +
					" " +
					data.info.credencial_apellido +
					" " +
					data.info.credencial_apellido_2
				);
				$(".box_correo_electronico").html(data.info.credencial_login);
				$(".box_celular_estudiante").html(data.info.celular);
			} else {
				Swal.fire({
					icon: "error",
					title: data.info,
...
// function mostrardatos(id_credencial){
// 	$.post("../controlador/calificarhuella.php?op=mostrardatos",{id_credencial : id_credencial}, function(data, status)
// 	{
...
function mostrarmaterias(id_programa_ac, id_estudiante, ciclo) {
	...
	$("#precarga").show();
	$.post(
		"../controlador/calificarhuella.php?op=mostrarmaterias",
		{
...
function matriculaprograma(id_credencial) {
	$.post(
		"../controlador/calificarhuella.php?op=matriculaprograma",
		{ id_credencial: id_credencial },
		function (data, status) {
			data = JSON.parse(data);
			$("#mostrardatos").html("");
...
function mostrar(id) {
	$.post(
		"../controlador/calificarhuella.php?op=mostrar",
		{ id: id },
		function (data, status) {
			data = JSON.parse(data);
			mostrarform(true);
...
function guardaryeditar(e) {
	...
	$.ajax({
		url:
			"../controlador/calificarhuella.php?op=guardaryeditar&credencial_identificacion=" +
			credencial,
		type: "POST",
		data: formData,
		contentType: false,
...
function matriculaMateriaNormal(
	id_estudiante,
	id_programa_ac,
	id_materia,
	semestres_del_programa,
	fecha
) {
	$("#precarga").show();
	$.post(
		"../controlador/calificarhuella.php?op=matriculaMateriaNormal",
		{
			id_estudiante: id_estudiante,
			id_materia: id_materia,
			semestres_del_programa: semestres_del_programa,
			fecha: fecha
		},
		function (data, status) {
			data = JSON.parse(data);
...
function eliminarMateria(
	...
	id_materia,
	semestres_del_programa,
	id_materia_matriculada,
	promedio_materia_matriculada
) {
	$("#precarga").show();
	$.post(
		"../controlador/calificarhuella.php?op=eliminarMateria",
		{
			id_estudiante: id_estudiante,
			id_materia: id_materia,
			semestres_del_programa: semestres_del_programa,
			id_materia_matriculada: id_materia_matriculada,
			promedio_materia_matriculada: promedio_materia_matriculada
		},
		function (data, status) {
			data = JSON.parse(data);
			if (data == true) {
...
function actualizarCambioJornada(e2) {
	...
	$.ajax({
		url: "../controlador/calificarhuella.php?op=actualizarJornada",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function actualizarCambioPeriodo(e3) {
	...
	$.ajax({
		url: "../controlador/calificarhuella.php?op=actualizarPeriodo",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function actualizarCambioGrupo(e4) {
	...
	$.ajax({
		url: "../controlador/calificarhuella.php?op=actualizarGrupo",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function nota(val, c, id, pro, tl) {
	if (val.length <= 4) {
		if (val <= 5.0 && val >= 0) {
			//console.log(nota);
			$.post(
				"../controlador/calificarhuella.php?op=nota",
				{ id: id, nota: val, tl: tl, c: c, pro: pro },
				function (data) {
					//console.log(data);
...
function promedio(val, c, id) {
	if (val.length <= 4) {
		if (val <= 5.0 && val >= 0) {
			// console.log(nota);
			$.post(
				"../controlador/calificarhuella.php?op=promedio",
				{ id: id, val: val, c: c },
				function (data) {
					// console.log(data);
					var r = JSON.parse(data);
...
function huella(valor, id, c) {
	$.post(
		"../controlador/calificarhuella.php?op=huella",
		{ id: id, val: valor, c: c },
		function (data) {
			// console.log(data);
			var r = JSON.parse(data);
