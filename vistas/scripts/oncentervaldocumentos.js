...
function init() {
	...
	$.post("../controlador/oncentervaldocumentos.php?op=selectEscuela", function (r) {
		$("#escuela").html(r);
...
function selectPrograma() {
	...
	$.post("../controlador/oncentervaldocumentos.php?op=selectPrograma", function (r) {
		$("#fo_programa").html(r);
...
function selectJornada() {
	//Cargamos los items de los selects
	$.post("../controlador/oncentervaldocumentos.php?op=selectJornada", function (r) {
		$("#jornada_e").html(r);
...
function selectTipoDocumento() {
	//Cargamos los items de los selects
	$.post("../controlador/oncentervaldocumentos.php?op=selectTipoDocumento", function (r) {
...
function selectNivelEscolaridad() {
	//Cargamos los items de los selects
	$.post("../controlador/oncentervaldocumentos.php?op=selectNivelEscolaridad", function (r) {
	...
	tabla = $('#tbllistado').dataTable(
		{
			...
			"ajax":
			{
				url: '../controlador/oncentervaldocumentos.php?op=listar',
				type: "get",
				dataType: "json",
				error: function (e) {
...
function mostrar(id_programa) {
	$.post("../controlador/oncentervaldocumentos.php?op=mostrar", { id_programa: id_programa }, function (data, status) {
...
function perfilEstudiante(id_estudiante, identificacion, fila) {
	...
	$("#fila").val(fila);
	$.post("../controlador/oncentervaldocumentos.php?op=perfilEstudiante", { id_estudiante: id_estudiante }, function (data, status) {
		data = JSON.parse(data);
...
function editarPerfil(e5) {
	...
	$.ajax({
		url: "../controlador/oncentervaldocumentos.php?op=editarPerfil",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function validarDocumentos(id_estudiante) {
	alertify.confirm('validar Documentos', '¿Está Seguro de validar los documentos?', function () {
		$.post("../controlador/oncentervaldocumentos.php?op=validarDocumentos", { id_estudiante: id_estudiante }, function (e) {
...
function verSoportes(id_estudiante) {
	$.post("../controlador/oncentervaldocumentos.php?op=verSoportes", { id_estudiante: id_estudiante }, function (e) {
...
function validar(id_estudiante, soporte) {
	$.post("../controlador/oncentervaldocumentos.php?op=validar", { id_estudiante: id_estudiante, soporte: soporte }, function (e) {
		e = JSON.parse(e);// convertir el mensaje a json
		if (e == 1) {
