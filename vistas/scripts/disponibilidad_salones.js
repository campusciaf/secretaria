...
function actulizarEstado() {
    $.ajax({ "url": "../controlador/disponibilidad_salones.php?op=actualizarEstado", "type": "POST", success: function (e) { } });
}
//lista todas las horas iniciales y finales
function listarHoras() {
    $.post("../controlador/disponibilidad_salones.php?op=selectHora", function (r) {
        $("#hora, #hasta").html(r);
...
function ajustarhasta(hora) {
    $.post("../controlador/disponibilidad_salones.php?op=selectHasta", { hora: hora }, function (r) {
        $("#hasta").html(r);
...
function consultarSalones(sede) {
    ...
    $.post("../controlador/disponibilidad_salones.php?op=consultarSalones", { sede: sede }, function (datos) {
...
function iniciarCalendario(codigo_salon, estado_formulario) {
    ...
    $('#calendar').fullCalendar({
        ...
        events: function (start, end, timezone, callback) {
            $.ajax({
                url: '../controlador/disponibilidad_salones.php?op=iniciarcalendario',
                type: 'POST',
                data: {
        ...
        dayClick: function (date) {
            ...
            if (estado_formulario == 1 && diffHoras <= 72) {
                ...
                var selectedHour = date.hour(); // Hora seleccionada (en formato de 24 horas)
                var selectedMinute = date.minute(); // Minutos 

                // Redondear el minuto de inicio al múltiplo de 5 más cercano
                selectedMinute = Math.floor(selectedMinute / 5) * 5;
                if (selectedMinute === 60) {  // Si los minutos redondean a 60, ajustarlos
                    selectedMinute = 0;
                    selectedHour += 1;  // Sumamos 1 a la hora
                }
                // Establecer la hora de inicio en el campo "startTime"
                ...
                var formattedEndTime = ("0" + selectedHour).slice(-2) + ":" + ("0" + selectedMinute).slice(-2);
                ...
                $("#endTime").on("input", function () {
                    ...
                    if (minutes % 5 !== 0) {
                        ...
                        var correctedTime = parts[0] + ":" + ("0" + correctedMinutes).slice(-2);
...
function aggReserva() {
    ...
    $.ajax({
        "type": "POST",
        "url": "../controlador/disponibilidad_salones.php?op=aggReserva",
        "data": formData,
        "contentType": false,
        "processData": false,
...
function misReservas() {
    ...
    $('#table-misreservas').dataTable({
        "aProcessing": true,//Activamos el procesamiento del datatables
        "aServerSide": true,//Paginación y filtrado realizados por el servidor
        "stateSave": true,
        "dom": "tp",
        "ajax": {
            "url": '../controlador/disponibilidad_salones.php?op=listarmisreservas&guia=r',
            "type": "get",
            "dataType": "json",
            error: function (e) {
...
function cancelarReservas(id, salon) {
    ...
    $.ajax({
        url: "../controlador/disponibilidad_salones.php?op=cancelarReserva",
        type: "POST",
        data: formData,
        cdataType: 'json',
        success: function (datos) {
...
function historialReservas() {
    ...
    $('#table_histo_reservas').dataTable({
        "aProcessing": true,//Activamos el procesamiento del datatables
        "aServerSide": true,//Paginación y filtrado realizados por el servidor
        "stateSave": true,
        "dom": "tp",
        "ajax": {
            "url": '../controlador/disponibilidad_salones.php?op=historialReservas',
            "type": "get",
            "dataType": "json",
            error: function (e) {
                // console.log(e.responseText);
...
function listarHoraSalones(salon) {
    $.post("../controlador/disponibilidad_salones.php?op=listarHoraSalones&salon=" + salon, function (datos) {
        var r = JSON.parse(datos);
        //console.log(r);
...
function listarSalones(jornada, fe) {
    ...
    $.post('../controlador/disponibilidad_salones.php?op=listarSalones&jornada=' + jornada + '&cantidad=' + cantidad + "&fecha=" + fechaEnv, function (data) {
        //console.log(data);
        var r = JSON.parse(data);
        $(".imprime_table").html(r.table);

        $('#table_listar_salones').DataTable({ "iDisplayLength": 30, "ordering": false });
...
function reservaSalon(id_hora, id) {
    ...
    $('td').click(function () {
        ...
        $.post('../controlador/disponibilidad_salones.php?op=consultaSalon', { "piso": piso }, function (data) {
            //console.log(data);
            var r = JSON.parse(data);
...
function consultaHoraI(hora) {
    ...
    $.ajax({
        url: "../controlador/disponibilidad_salones.php?op=consultaHoraI",
        type: "POST",
        data: data,
        cdataType: 'json',
        success: function (r) {
            var datos = JSON.parse(r);
            $("#h_inicio").html('<option value="' + datos.id_horas + '">' + datos.formato + '</option>');
        }
    });
}
...
function consultaHoraF(hora, jornada) {
    ...
    $.ajax({
        url: "../controlador/disponibilidad_salones.php?op=consultaHoraF",
        type: "POST",
        data: data,
        cdataType: 'json',
        success: function (r) {
            ...
            var opti = "";
            //console.log(datos);
            opti += '<option value="" selected disabled > -Seleccione la hora final-</option>';
            for (index = 0; index < datos.length; index++) {
                opti += '<option value="' + datos[index].id_horas + '">' + datos[index].formato + '</option>';
            }
...
function mostrar_datos_personales_usuarios() {
    $.post("../controlador/disponibilidad_salones_admin.php?op=mostrar_datos_personales_usuarios", {}, function (data) {
...
function finalizaReservas() {
    ...
    $('#table_reservas_finali').dataTable(
        {
            "aProcessing": true,//Activamos el procesamiento del datatables
            "aServerSide": true,//Paginación y filtrado realizados por el servidor
            responsive: true,
            ...
            "ajax":
            {
                url: '../controlador/disponibilidad_salones.php?op=listarmisreservas&guia=f',
                type: "get",
                dataType: "json",
                error: function (e) {
                }
