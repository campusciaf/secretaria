...
require_once "../modelos/HvFunDocumentosObligatorios.php";
$documentosObligatorios = new HvFunDocumentosObligatorios();
...
$id_cvadministrativos_documento_obligatorio = isset($_POST['id_cvadministrativos_documento_obligatorio']) ? $_POST['id_cvadministrativos_documento_obligatorio'] : "";
...
switch ($_GET['op']) {
    case 'guardaryeditardocumentos_obligatorios':
        ...
        // $rspta = $documentosObligatorios->insertarDocumentosO($id_cvadministrativos_documento_obligatorio, $nombre_tipos_documentos);
        ...
        //     if ($documentosObligatorios->existeDocumentoUsuario($id_cvadministrativos_documento_obligatorio, $nombre_documento_limpio)) {
        //         echo json_encode([
        //             "estatus" => 0,
        ...
        //     $estado = 1;
        //     $rspta = $documentosObligatorios->InsertarDocumentosRutTarjetaProfesional($nombre_documento_limpio, $estado, $id_cvadministrativos_documento_obligatorio);
        // } else {
        //     $rspta = $documentosObligatorios->insertarDocumentosO($id_cvadministrativos_documento_obligatorio, $nombre_tipos_documentos);
        // }
        // solo entra si es rut o tarjeta profesional
        if ($nombre_documento_limpio === "Registro Único Tributario" || $nombre_documento_limpio === "Tarjeta Profesional") {
            // verificamos si ya existe.
            $id_existente = $documentosObligatorios->obtenerIdDocumentoUsuario($id_cvadministrativos_documento_obligatorio,$nombre_documento_limpio);
            if ($id_existente) {
        ...
        if ($rspta) {
            ...
            $carpeta_destino = '../cv_funcionarios/documentos/';



            if ($file_type == "application/pdf") {
                $img1path = $carpeta_destino . "documento_U" . $id_cvadministrativos . "_O" . $rspta . ".pdf";
            } else {
                $img1path = $carpeta_destino . "documento_U" . $id_cvadministrativos . "_O" . $rspta . ".jpg";
            }
    ...
    case 'mostrarDocumentosObligatorios':
        ...
        for ($i = 0; $i < $documentos_count; $i++) {
            ...
            $documentos_usuario = $documentosObligatorios->listarDocumentosObligatorios($id_cvadministrativos, $nombre_documentos_filtrado);
            if (is_array($documentos_usuario) && count($documentos_usuario) > 0 && !empty($documentos_usuario[0]['documento_archivo'])) {
                ...
                $crear_documento = '';
                $boton_eliminar = '<button class="btn btn-flat btn-danger btn-sm" onclick="eliminarDocumentoObligatorio(' . $id_cvadministrativos . ',`' . $documentos_usuario[0]['documento_nombre'] . '`)" data-toggle="tooltip" data-placement="bottom" title="Eliminar">  <i class="fas fa-trash"></i> </button>';
                $switch = '';
            } else {
                $ruta_documento_obligatorio = 'Falta Subir';
                $crear_documento = '<button class="btn btn-primary" onclick="CrearDocumentoObligatorio(' . $id_cvadministrativos . ',`' . $nombre_documentos . '`)">Subir Archivo</button>';
                $boton_eliminar = '';
                $switch = '';

                if ($nombre_documentos === "Registro Único Tributario (RUT)" || $nombre_documentos === "Tarjeta Profesional") {
                    $crear_documento = '<button id="btnSubir_' . $i . '" class="btn btn-primary d-none" onclick="CrearDocumentoObligatorio(' . $id_cvadministrativos . ',`' . $nombre_documentos . '`)">Subir Archivo</button>';
                    ...
                    <div class="d-flex justify-content-center align-items-center mb-3" style="gap: 1rem;">
                        <div style="text-align: center !important;">
                            ...
                            <input class="form-check-input" type="radio" name="toggle_' . $i . '" id="toggle_si_' . $i . '" onchange="toggleSubir(' . $i . ', true, ' . $id_cvadministrativos . ',`' . $nombre_documentos . '` )">
                            <label for="toggle_si_' . $i . '" class="switch-label switch-label-off" >Sí</label>
                            </div>
                            <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="toggle_' . $i . '" id="toggle_no_' . $i . '"  onchange="toggleSubir(' . $i . ', false, ' . $id_cvadministrativos . ',`' . $nombre_documentos . '`)" checked>
    ...
    case 'eliminarDocumentoObligatorio':
        ...
        $rspta = $documentosObligatorios->cvdocumentosOeliminar($id_cvadministrativos, $nombre_documentos);
        if ($rspta) {
            $inserto = array(
                "estatus" => 1,
    ...
    case 'mostrarotrosestudios':
        ...
        $documentos_usuario = $documentosObligatorios->listarDocumentosObligatoriosotrosestudios($id_cvadministrativos);
    ...
    case 'actualizar_porcentaje_avance':
        ...
        $conteo = $documentosObligatorios->CuentoRegistros($id_cvadministrativos);
        $totalRegistros = isset($conteo['total']) ? intval($conteo['total']) : 0;
        if ($porcentaje_actual < 77) {
            $nuevo_porcentaje = 77;
            $documentosObligatorios->actualizar_porcentaje_personal($nuevo_porcentaje, $id_cvadministrativos);
        }
        $inserto = array(
            "estatus" => 1,
        ...
        break;
    // actualizamos el estado para tarjeta profesional y rut
    case 'ActualizarEstadoRutTarjetaProfesional':
        ...
        // if ($documentosObligatorios->existeDocumentoUsuario($id_cvadministrativos, $nombre_documento_limpio)) {
        //     echo json_encode([
        //         "estatus" => 0,
        //         "valor" => "Este documento ya fue registrado."
        //     ]);
        //     exit();
        // }
        // $rspta = $documentosObligatorios->InsertarDocumentosRutTarjetaProfesional($nombre_documento_limpio, $estado, $id_cvadministrativos);
        // verificar si el documento ya existe
        if ($documentosObligatorios->existeDocumentoUsuario($id_cvadministrativos, $nombre_documento_limpio)) {
            //en caso de que exista se actualiza el documento.
            $rspta = $documentosObligatorios->ActualizarEstadoRutTarjetaProfesional( $nombre_documento_limpio, $estado, $id_cvadministrativos);
        } else {
