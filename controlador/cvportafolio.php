...
session_start();
require_once "../modelos/CvPortafolio.php";
$Portafolio = new CvPortafolio();
...
switch ($_GET['op']) {
    case 'guardaryeditarportafolio':
        ...
        if (empty($id_portafolio)) {
            $rspta = $Portafolio->cvinsertarPortafolio($id_usuario_cv, $titulo_portafolio, $video_portafolio, $descripcion_portafolio,  "imagen");
            if ($rspta) {
                $conteo = $Portafolio->CuentoRegistros($id_usuario_cv);
                ...
                if ($porcentaje_actual < 50) {
                    ...
                    $nuevo_porcentaje = 50;
                    $Portafolio->actualizar_porcentaje_personal($nuevo_porcentaje, $id_usuario_cv);
                }
                $carpeta_destino = '../cv/files/portafolio/';
                $url_imagen = '../files/portafolio/';
                if ($file_type == "application/pdf") {
                    $img1path = $carpeta_destino . "Portafolio_U" . $id_usuario . "_P" . $rspta . ".pdf";
                    $url_imagen = $url_imagen . "Portafolio_U" . $id_usuario . "_P" . $rspta . ".pdf";
                } else {
                    $img1path = $carpeta_destino . "Portafolio_U" . $id_usuario . "_P" . $rspta . ".jpg";
                    $url_imagen = $url_imagen . "Portafolio_U" . $id_usuario . "_P" . $rspta . ".jpg";
                }
