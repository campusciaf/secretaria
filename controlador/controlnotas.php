<?php 
session_start();
require_once "../modelos/ControlNotas.php";
$cuenta_docente = new ControlNotas();
...
switch ($_GET["op"]){
    case 'listar':
		...
		for ($i=0; $i<count($reg); $i++){
			...
			$nombremateria=$cuenta_docente->nombremateria($id_materia);

			$data[] = array(
				"0"=>$reg[$i]["usuario_nombre"] . " " . $reg[$i]["usuario_nombre_2"] . " " . $reg[$i]["usuario_apellido"] . " " . $reg[$i]["usuario_apellido_2"],
				"1"=>$reg[$i]["usuario_identificacion"],
				"2"=>$reg[$i]["jornada"],
				"3"=>$nombremateria["nombre"],
				"4"=>$reg[$i]["nombre"],
				"5"=>($reg[$i]["entregada"] == 1)?'<span class="badge badge-success">Entregado</span>':'<button class="btn btn-danger btn-sm btn-flat" onClick="cambiarEntregada('.$reg[$i]["id_docente_grupo"].')"> Entregar</button>',
				"6"=> $reg[$i]["semestre"]
			);
		}
		$results = array(
	...
	case 'listarEstudiantes':
		...
		$data["0"] .= '<table id="estudiantes_grupo" class="table table-hover table-nowarp" style="width:100%">
            <thead>
                <tr>
		...
		for ($i=0;$i<count($info_estudiantes);$i++){
			...
			if (file_exists('../files/estudiantes/'.$datos_credencial["credencial_identificacion"].'.jpg')) {
				$foto='<img src=../files/estudiantes/'.$datos_credencial["credencial_identificacion"].'.jpg width=50px height=50px class=img-circle>';
			}else{
            ...
            $data["0"] .= '<tr>
				<td><input type="hidden" value="'.$materia.'" id="materia">'.
					$foto.'  '.$datos_credencial["credencial_identificacion"].' | '.$estado .' | '. $caracterizado.'
				</td>
				<td>'.
					$datos_credencial["credencial_nombre"]. ' ' . $datos_credencial["credencial_nombre_2"].' </td>
					<td>' . $datos_credencial["credencial_apellido"]. ' ' . $datos_credencial["credencial_apellido_2"].' ' .'
				</td>
	...
	case 'cambiarEntregada':
		//recibiendo informacion desde el javascript
		$id_docente_grupo =$_POST["id_docente_grupo"];
		//enviando consulta al modelo por medio del objeto de la clase 
		$rpsta = $cuenta_docente->cambiarEntregada($id_docente_grupo);
		if($rpsta){
			$data = array("exito" => 1, "info" => "Entregada con exito");
		}else{
			//es por que no se realizo el cambio y hubo un error
			$data = array("exito" => 0, "info" => "Error, la consulta falló");
		}
		//como $data es un array debemos de enviarla como JSON
		//json_encode = convierte arrays en json
		echo json_encode ($data);
	break;
