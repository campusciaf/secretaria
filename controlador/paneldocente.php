<?php
require_once "../modelos/PanelDocente.php";
...
$rsptaperiodo = $paneldocente->periodoactual();
...
switch ($_GET["op"]) {
	...
	case 'guardaryeditar':
		...
		if ($_SESSION['usuario_cargo'] == "Docente") {
			$rspta = $paneldocente->insertarEncuestaDocente($id_usuario, $r1, $r2, $r3, $r4, $r5, $r6, $r7, $r8, $r9, $r10, $fecha, $hora, $periodo_actual);
			$paneldocente->actualizarEstadoAutoevaluacion($id_usuario);
			$data["0"] .= 'd';
		} else {
			// $estudiante = $paneldocente->consultaDatos($id_usuario);
			// $identificacion=$estudiante["credencial_identificacion"];
			// $programa=$estudiante["fo_programa"];
			// $jornada=$estudiante["jornada_e"];	
			// $rspta=$paneldocente->insertarEncuestaEstudiante($id_usuario,$identificacion,$programa,$jornada,$fecha,$hora,$r1,$r2,$r3,$r4);		
			// $data["0"] .= 'e';
		}
		$results = array($data);
	...
	case 'verificarAutoevaluacion':
		$valor = "-";
		$estado = $paneldocente->consultarEstadoAutoevaluacion('autoevaluacion')["estado"];
		if ($_SESSION['usuario_cargo'] == "Docente" and $estado == 1) {
			$rspta = $paneldocente->autoevaluacionEstado($id_usuario, $periodo_actual);
			if ($rspta == true) {
				$valor = 1;
			} else {
				$valor = 0;
	...
	case 'alertacalendario':
		...
		$fecha_actual = isset($_POST["fecha_actual"]) ? limpiarCadena($_POST["fecha_actual"]) : "";
		$alerta = $paneldocente->alertacalendario();
		...
		for ($r = 0; $r < count($alertacalendario); $r++) {
			...
			$data[0] .= '
				...
				<b>' . $nombre . '<br> </b>	
				<br><b> Fecha de Inicio :</b> ' . $paneldocente->fechaesp($alertacalendario[$r]["fecha_inicio"]) .

				'<br><b>Fecha de Final : </b>' . $paneldocente->fechaesp($alertacalendario[$r]["fecha_final"]) . '<hr>
				</div>';
		};
	...
	case 'verperfilactualizado':
		...
		$rspta = $paneldocente->verperfilactualizado($id_usuario, $semanas);
	...
	case 'actualizarperfil':
		$data = array();
		$data["estado"] = "";
		$rspta = $paneldocente->actualizarperfil($id_usuario, $usuario_direccion, $usuario_telefono, $usuario_celular, $usuario_email, $fecha);
		$rspta ? "si" : "no";
	...
	case 'mostrar':
		$rspta = $paneldocente->mostrar($id_usuario);
	...
	case 'listardatos':
		...
		switch ($rango) {
			case '1':
				$rspta1 = $paneldocente->listaringresos($id_usuario, $fecha);
				$rspta2 = $paneldocente->listarfaltas($id_usuario, $fecha);
				$rspta3 = $paneldocente->listar_actividades($id_usuario, $fecha);
				$rspta4 = $paneldocente->listarguia($id_usuario, $fecha);
				$rspta5 = $paneldocente->perfilactualizadodocente($fecha, $id_usuario);
				$rspta6 = $paneldocente->usuariohojadevida($fecha);


				$rspta1 = $paneldocente->fechaperfilactualizado($id_usuario);

				// $perfilactualizado=$rspta1["fecha_actualizacion"];

				// $fechaperfilactualizado=$paneldocente->fechaesp($perfilactualizado);

				$listarhojasdevida = $paneldocente->datosDocente($id_usuario);
				for ($n = 0; $n < count($listarhojasdevida); $n++) {
					$usuario_identificacion = $listarhojasdevida["usuario_identificacion"];
				}
				$usuario_cv = $paneldocente->usuario_cv($usuario_identificacion);
				$id_usuario_cv = $usuario_cv["id_usuario_cv"];

				$cv_informacion_personal = $paneldocente->usuario_cv_personal($id_usuario_cv);

				$ultima_actualizacion_hj = $cv_informacion_personal["ultima_actualizacion"];

				$actualizacion_hoja_de_vida = $paneldocente->fechaesp($ultima_actualizacion_hj);


				// creamos el contador
				$contador = 0;
				$listar_grupos = $paneldocente->listargrupos($id_usuario);
				for ($i = 0; $i < count($listar_grupos); $i++) {
					...
					$rsptaprograma = $paneldocente->programaacademico($id_programa);
					// contiene el nombre de la jornada
					$jornada = $listar_grupos[$i]["jornada"];
					// consulta para traer el nombre del programa
					$rsptajornada = $paneldocente->jornada($jornada);
					// consulta para traer el formato de la hora
					$hora = $listar_grupos[$i]["hora"];
					$rsptahora = $paneldocente->horasFormato($hora);
					// consulta para traer el formato de hasta
					$hasta = $listar_grupos[$i]["hasta"];
					$rsptahasta = $paneldocente->horasFormato($hasta);

					$listarelgrupo = $paneldocente->listarelgrupo($listar_grupos[$i]["id_docente_grupo"]);
					...
					$datosmateriaciafi = $paneldocente->MateriaDatos($id_materia); // consuta pra traer los datos de la materia
					$materia = $datosmateriaciafi["nombre"]; // nombre de la materia

					//se traen las materias para contarlas y mostraslas en el panel
					$listar_materias = $paneldocente->listar_materias($ciclo, $materia, $jornada, $id_programa, $grupo);
			...
			case '2':
				...
				$rspta1 = $paneldocente->listaringresos($id_usuario, $fecha_anterior);
				$rspta2 = $paneldocente->listarfaltas($id_usuario, $fecha_anterior);
				$rspta3 = $paneldocente->listar_actividades($id_usuario, $fecha_anterior);
				$rspta4 = $paneldocente->listarguia($id_usuario, $fecha_anterior);
				$rspta5 = $paneldocente->perfilactualizadodocente($fecha_anterior, $id_usuario);
				$rspta6 = $paneldocente->usuariohojadevida($fecha_anterior);
			...
			case '3':
				...
				$rspta1 = $paneldocente->listaringresossemana($id_usuario, $semana);
				$rspta2 = $paneldocente->listarfaltassemana($id_usuario, $semana);
				$rspta3 = $paneldocente->listar_pea_actividades_semana($id_usuario, $semana);
				$rspta4 = $paneldocente->listarguiasemana($id_usuario, $semana);
				$rspta5 = $paneldocente->perfilactualizadodocente($semana, $id_usuario);
				$rspta6 = $paneldocente->usuariohojadevida($semana);
			...
			case '4':
				...
				$rspta1 = $paneldocente->listaringresossemana($id_usuario, $mes_actual);
				$rspta2 = $paneldocente->listarfaltassemana($id_usuario, $mes_actual);
				$rspta3 = $paneldocente->listar_pea_actividades_semana($id_usuario, $mes_actual);
				$rspta4 = $paneldocente->listarguiasemana($id_usuario, $mes_actual);
				$rspta5 = $paneldocente->perfilactualizadodocente($mes_actual, $id_usuario);
				$rspta6 = $paneldocente->usuariohojadevida($mes_actual);
			...
			case '5':
				...
				$rspta1 = $paneldocente->listaringresosrango($id_usuario, $fecha_inicial, $fecha_final);
				$rspta2 = $paneldocente->listarfaltasrango($id_usuario, $fecha_inicial, $fecha_final);
				$rspta3 = $paneldocente->listaractividades($id_usuario, $fecha_inicial, $fecha_final);
				$rspta4 = $paneldocente->listarguiarango($id_usuario, $fecha_inicial, $fecha_final);
				$rspta5 = $paneldocente->perfilactualizadodocenterango($id_usuario, $fecha_inicial, $fecha_final);
				$rspta6 = $paneldocente->usuariohojadevidarango($fecha_inicial, $fecha_final);
				// creamos el contador
				$contador = 0;
				$listar_grupos = $paneldocente->listargrupos($id_usuario);
				for ($i = 0; $i < count($listar_grupos); $i++) {
	...
	case 'mostrar_nombre_docente':
		...
		switch ($rangodocente) {
			case 1:
				...
				$totaldocente = $paneldocente->listaringresos($id_usuario, $fecha);

				for ($n = 0; $n < count($totaldocente); $n++) {
					// $totaldocente = $paneldocente->mostrardocente($id_usuario, $fecha);
					// Datos Funcionario 
					...
					$data[0] .= '
							...
							<tr>
								...
								<td>' . $paneldocente->fechaespcalendario($fecha) . '</td> 
								<td>' . $hora . '</td>  
							</tr>
						';
				}
			...
			case 2:
				...
				<tbody>';
				$totaldocente = $paneldocente->listaringresos($id_usuario, $fecha_anterior);
				// print_r($totaldocente);
				for ($n = 0; $n < count($totaldocente); $n++) {
					// $totaldocente = $paneldocente->mostrardocente($id_usuario, $fecha_anterior);
					// Datos Funcionario 
					$usuario_identificacion = $totaldocente[$n]["usuario_identificacion"];
					$usuario_nombre = $totaldocente[$n]["usuario_nombre"];
					$usuario_nombre_2 = $totaldocente[$n]["usuario_nombre_2"];
			...
			case 3:
				...
				<tbody>';
				$totaldocente = $paneldocente->mostrardocentesemana($id_usuario, $semana);
				for ($n = 0; $n < count($totaldocente); $n++) {
			...
			case 4:
				...
				<tbody>';
				// $totaldocente=$paneldocente->listaringresos($id_usuario,$mes_actual);
				$totaldocente = $paneldocente->mostrardocentesemana($id_usuario, $mes_actual);

				for ($n = 0; $n < count($totaldocente); $n++) {
					// $totaldocente = $paneldocente->mostrardocente($id_usuario, $mes_actual);
					// Datos Docente 
					$usuario_identificacion = $totaldocente[$n]["usuario_identificacion"];
					$usuario_nombre = $totaldocente[$n]["usuario_nombre"];
					$usuario_nombre_2 = $totaldocente[$n]["usuario_nombre_2"];
			...
			case 5:
				...
				$totaldocente = $paneldocente->listaringresosrangonombredocente($id_usuario, $fecha_inicial, $fecha_final);
	...
	case 'mostrar_faltas':
		...
		switch ($rangofaltas) {
			case 1:
				...
				$mostrar_faltas = $paneldocente->mostrarfaltas($_SESSION['id_usuario'], $fecha);
			...
			case 2:
				...
				$mostrar_faltas = $paneldocente->mostrarfaltas($_SESSION['id_usuario'], $fecha_anterior);
			...
			case 3:
				...
				$mostrar_faltas = $paneldocente->mostrarfaltassemana($_SESSION['id_usuario'], $semana);
			...
			case 4:
				...
				$mostrar_faltas = $paneldocente->mostrarfaltassemana($_SESSION['id_usuario'], $mes_actual);
			...
			case 5:
				...
				$mostrar_faltas = $paneldocente->listarfaltasrangonombre($_SESSION['id_usuario'], $fecha_inicial, $fecha_final);
		...
		switch ($actividadesnuevas) {
			case 1:
				...
				$data[0] .=
					...
					<tbody></thead>';
				$documentos_pea = $paneldocente->listar_actividades($id_usuario, $fecha);
			...
			case 2:
				...
				$data[0] .=
					...
					<tbody></thead>';
				$documentos_pea = $paneldocente->listar_actividades($id_usuario, $fecha_anterior);
				for ($h = 0; $h < count($documentos_pea); $h++) {
					$nombre_documento = $documentos_pea[$h]["nombre_documento"];
					$descripcion_documento = $documentos_pea[$h]["descripcion_documento"];
					$archivo_documento = $documentos_pea[$h]["archivo_documento"];
			...
			case 3:
				...
				$data[0] .=
					...
					<tbody></thead>';
				$documentos_pea = $paneldocente->listar_pea_actividades_semana($id_usuario, $semana);
				for ($h = 0; $h < count($documentos_pea); $h++) {
					$nombre_documento = $documentos_pea[$h]["nombre_documento"];
					$descripcion_documento = $documentos_pea[$h]["descripcion_documento"];
					$archivo_documento = $documentos_pea[$h]["archivo_documento"];
			...
			case 4:
				...
				$data[0] .=
					...
					<tbody></thead>';
				$documentos_pea = $paneldocente->listar_pea_actividades_semana($id_usuario, $mes_actual);
				for ($h = 0; $h < count($documentos_pea); $h++) {
					$nombre_documento = $documentos_pea[$h]["nombre_documento"];
					$descripcion_documento = $documentos_pea[$h]["descripcion_documento"];
					$archivo_documento = $documentos_pea[$h]["archivo_documento"];
			...
			case 5:
				...
				$caracterizacion = $paneldocente->listar_actividades_por_fecha($id_usuario, $fecha_inicial, $fecha_final);
	...
	case 'siguienteclase':
		...
		switch ($siguienteclase) {
			case 1:
				...
				$reg = $paneldocente->listargrupos_dia_actual($id_usuario, $array[$num_dia]);

				for ($i = 0; $i < count($reg); $i++) {

					$datosmateriaciafi = $paneldocente->materiaDatos($reg[$i]["id_materia"]); // consuta pra traer los datos de la materia
					...
					// consulta para traer el formato de la hora
					$hora = $reg[$i]["hora"];
					$inicio_clase = $paneldocente->horasFormato($hora);
	...
	case 'casosquedate':
		...
		switch ($casosquedate) {
			case 1:
				...
				// $casoquedate=$paneldocente->casoquedate($fecha);
				$casoquedate = $paneldocente->listarguia($id_usuario, $fecha);
			...
			case 2:
				...
				$casoquedate = $paneldocente->listarguiaayer($id_usuario, $fecha_anterior);
				// print_r($casoquedate);
				for ($n = 0; $n < count($casoquedate); $n++) {
			...
			case 3:
				...
				$casoquedate = $paneldocente->listarguiatablasemana($id_usuario, $semana);
				for ($n = 0; $n < count($casoquedate); $n++) {
			...
			case 4:
				...
				$casoquedate = $paneldocente->listarguiatablasemana($id_usuario, $mes_actual);
				for ($n = 0; $n < count($casoquedate); $n++) {
			...
			case 5:
				...
				// $casoquedate=$paneldocente->listarguiatabla($id_usuario,$mes_actual);
				$casoquedate = $paneldocente->listarguiarango5($id_usuario, $fecha_inicial, $fecha_final);
				for ($n = 0; $n < count($casoquedate); $n++) {
	...
	case 'perfilesactualizados':
		...
		switch ($perfilactualizado) {
			...
			case 1:
				...
				// $perfilactualizado=$paneldocente->perfilesactualizados($fecha);

				$docente = $paneldocente->perfilactualizadodocente($fecha, $id_usuario);
					...
					$data[0] .= '
								...
								<tr>
									<th scope="row" class="text-center">' . ($n + 1) . '</th>	
										...
										<td>' . $paneldocente->fechaespcalendario($fecha_actualizacion) . '</td> 
									</tr>
							';
				}
				$data[0] .= '</tbody></table>';
			...
			case 2:
				...
				// $perfilactualizado=$paneldocente->perfilesactualizados($fecha_anterior);

				$docente = $paneldocente->perfilactualizadodocenteayer($fecha_anterior, $id_usuario);
				for ($n = 0; $n < count($docente); $n++) {
			...
			case 3:
				...
				$docente = $paneldocente->perfilactualizadodocente($semana, $id_usuario);
			...
			case 4:
				...
				$docente = $paneldocente->perfilactualizadodocente($mes_actual, $id_usuario);
				for ($n = 0; $n < count($docente); $n++) {
			...
			case 5:
				...
				$docente = $paneldocente->perfilactualizadodocenterango($id_usuario, $fecha_inicial, $fecha_final);
				for ($n = 0; $n < count($docente); $n++) {
	...
	case 'hojadevidanueva':
		...
		switch ($hojasdevidanueva) {
			case 1:
				...
				$cv_informacion_personal = $paneldocente->usuario_cv_personal($id_usuario_cv, $fecha);
				...
				$data[0] .= '
						...
						<tbody>
							<tr>
								...
								<td>' . $paneldocente->fechaespcalendario($arreglo_fecha[0]) . '</td> 
								</tr>
						</tbody>';
			...
			case 2:
				...
				$usuario_identificacion = $usuario_cv["usuario_identificacion"];
				$nombre_docente = $usuario_cv["usuario_nombre"] . "" . $usuario_cv["usuario_apellido"];
				$cv_informacion_personal = $paneldocente->usuario_cv_personal($id_usuario_cv, $fecha_anterior);
				// $ultima_actualizacion=$cv_informacion_personal["ultima_actualizacion"];
			...
			case 3:
				...
				$usuario_identificacion = $usuario_cv["usuario_identificacion"];
				$nombre_docente = $usuario_cv["usuario_nombre"] . "" . $usuario_cv["usuario_apellido"];
				$cv_informacion_personal = $paneldocente->usuario_cv_personal($id_usuario_cv, $semana);
				// $ultima_actualizacion=$cv_informacion_personal["ultima_actualizacion"];
				// print_r($cv_informacion_personal);
			...
			case 4:
				...
				$usuario_identificacion = $usuario_cv["usuario_identificacion"];
				$nombre_docente = $usuario_cv["usuario_nombre"] . "" . $usuario_cv["usuario_apellido"];
				$cv_informacion_personal = $paneldocente->usuario_cv_personal($id_usuario_cv, $mes_actual);
				// $ultima_actualizacion=$cv_informacion_personal["ultima_actualizacion"];
				// print_r($cv_informacion_personal);
			...
			case 5:
				...
				$usuario_identificacion = $usuario_cv["usuario_identificacion"];
				$nombre_docente = $usuario_cv["usuario_nombre"] . "" . $usuario_cv["usuario_apellido"];
				$cv_informacion_personal = $paneldocente->usuario_cv_personal_rango($id_usuario_cv, $fecha_inicial, $fecha_final);
		...
		switch ($estudiantesacargo) {
			...
			case 1:
				$data["0"] .= '
						...
						';
				$reg = $paneldocente->listargrupos($id_usuario);
				// print_r($reg);
				for ($i = 0; $i < count($reg); $i++) {
					// print_r($reg);
					...
					$listarelgrupo = $paneldocente->listarelgrupo($reg[$i]["id_docente_grupo"]);
					...
					for ($j = 0; $j < count($listar_materias); $j++) {
						...
						$estudiante_id = $paneldocente->id_estudiante($listar_materias[$j]["id_estudiante"]);
						$id_credencial = $estudiante_id["id_credencial"];
						$datos_estudiante = $paneldocente->estudiante_datos($id_credencial);
	...
	case 'listarrango':
		...
		$rspta1 = $paneldocente->listaringresosrango($id_usuario, $fecha_inicial, $fecha_final);
		$rspta2 = $paneldocente->listarfaltasrango($id_usuario, $fecha_inicial, $fecha_final);
		$rspta3 = $paneldocente->listar_actividades_por_fecha($id_usuario, $fecha_inicial, $fecha_final);
		$rspta4 = $paneldocente->listarguiarango($id_usuario, $fecha_inicial, $fecha_final);
		$rspta5 = $paneldocente->perfilactualizadodocenterango($id_usuario, $fecha_inicial, $fecha_final);
		$rspta6 = $paneldocente->usuariohojadevidarango($fecha_inicial, $fecha_final);
		...
		for ($i = 0; $i < count($reg); $i++) {
			...
			//se traen las materias 
			$reg = $paneldocente->listar_materias($ciclo, $materia, $jornada, $id_programa, $grupo);
			for ($i = 0; $i < count($reg); $i++) {

				//listamos los estudiantes por materia
				$estudiantes = $paneldocente->id_estudiante($reg[$i]["id_estudiante"]);
			}
		}
	...
	case 'listardatosfijos':
		...
		$actualizacion_hoja_de_vida = $paneldocente->fechaesp($ultima_actualizacion);
	...
	case 'encuestatic':
		$valor = "-";
		if ($_SESSION['usuario_cargo'] == "Docente") {
			$rspta = $paneldocente->encuestatic($id_usuario, $periodo_actual);
			if ($rspta == true) {
				$valor = 1;
			} else {
				$valor = 0;
	...
	case 'encuestaticnueva':
		$valor = "-";
		if ($_SESSION['usuario_cargo'] == "Docente") {
			$rspta = $paneldocente->encuestaticpreguntas($id_usuario, $periodo_actual);
	...
	case 'encuestaticpreguntas':
		...
		$traerpregunta = $paneldocente->traerpregunta($totalrespuestas);
	...
	case 'guardarencuestatic':
		...
		$rspta = $paneldocente->insertarencuestatic($id_usuario, $fecha, $hora, $periodo_actual, $id_pregunta, $er1);
		if ($rspta == true) {
			$valor = 1;
		} else {
			$valor = 0;
		...
		// case 'guardarencuestatic2':
		//     $data= Array();
		// 	$valor = "";
		//     $promedio=(($er1+$er2+$er3+$er4+$er5+$er6+$er7+$er8+$er9+$er10+$er11+$er12+$er13+$er14+$er15+$er16+$er17+$er18+$er19+$er20+$er21+$er22+$er23+$er24+$er25+$er26+$er27+$er28+$er29+$er30+$er31+$er32+$er33+$er34+$er35+$er36+$er37+$er38+$er39+$er40+$er41+$er42+$er43+$er44+$er45)*100)/45;
		//     $rspta=$paneldocente->insertarencuestatic($id_usuario,$fecha,$hora,$periodo_actual,$er1,$er2,$er3,$er4,$er5,$er6,$er7,$er8,$er9,$er10,$er11,$er12,$er13,$er14,$er15,$er16,$er17,$er18,$er19,$er20,$er21,$er22,$er23,$er24,$er25,$er26,$er27,$er28,$er29,$er30,$er31,$er32,$er33,$er34,$er35,$er36,$er37,$er38,$er39,$er40,$er41,$er42,$er43,$er44,$er45,$promedio);
		//     if($rspta == true){
		//         $valor = 1;
		//     }else{
		//         $valor = 0;
	...
	case 'iniciarcalendario':
		...
		$traerhorario = $paneldocente->listarcalendarioacademico($fecha);
		$impresion .= '[';
	...
	case 'porcentaje_evaluacion':
		...
		$docente_grupos = $paneldocente->listarGruposPeriodo($id_docente, $periodo_actual);
		for ($j = 0; $j < count($docente_grupos); $j++) {
			$promedio = $paneldocente->listarResultados($docente_grupos[$j]['id_docente_grupo'], $periodo_actual);
			if (count($promedio) > 0) {
	...
	case 'comentarios_heteroevaluacion':
		$id_docente = $_SESSION["id_usuario"];
		$comentario = $paneldocente->listarComentarios($id_docente, $periodo_actual);
		//echo $periodo_actual;
	...
	case 'mostrarcalendario':
		...
		if (!empty($_POST['check_list'])) {
			...
			$cuenta = 0;
			$eventosacademicos = $paneldocente->traercalendario($anio_actual . "-" . $check);
	...
	case 'mostrarcalendarioeventos':
		...
		if (!empty($_POST['check_list'])) {
			...
			$eventos = $paneldocente->listarEventos($anio_actual . "-" . $check, $fecha_actual_eventos);
			...
			for ($d = 0; $d < count($eventos); $d++) {
				...
				$actividad_pertenece = $paneldocente->selectActividadActiva($eventos[$d]["id_actividad"], $check);
	...
	case 'checkbox':
		...
		if (!empty($_POST['check_list'])) {
			...
			// imprime las actividades del calendario 
			$eventosacademicos = $paneldocente->traercalendario($anio_actual . "-" . $check, $fecha_actual_eventos);
	...
	case 'cajadeherramientas':
		...
		$respuesta = $paneldocente->mostrarElementos();
	...
	case 'vershopping':
		...
		$respuesta = $paneldocente->mostrarshopping();
		for ($i = 0; $i < count($respuesta); $i++) {
			$data['vershopping'] .= '
	...
	case 'mostrarcursosinscritos':
		$data['conte'] = '';
		$miscursos = $paneldocente->mostrarcursosinscritos($s_identificacion);
	...
	case 'separarCupo':
		...
		$data['conte'] = '';
		$miscursos = $paneldocente->verificarInteresado($id_curso, $s_identificacion);
		if ($miscursos) {
			$data['conte'] .= 'si';
		} else {
			$estado = "Interesado";
			$insertarinteresado = $paneldocente->insertarInteresado($s_identificacion, $id_curso, $periodo_actual, $fecha, $hora, $estado, $periodo_actual, $id_usuario);
	...
	case 'eliminarInscripcion':
		$valor = $_POST["valor"];
		$data['conte'] = '';
		$miscursos = $paneldocente->eliminarinscripcion($valor);
	...
	case 'guardarmonitoreandodocente':
			...
			$fecha = date('Y-m-d');
			$verificar_id_usuario = $paneldocente->verificarRegistromonitoreandodocente($id_usuario);
			if ($verificar_id_usuario) {
				$inserto = array(
					"estatus" => 0,
					"valor" => "La información ya está insertada para este usuario y fecha"
				);
			} else {
				$rspta = $paneldocente->GuardarmonitoreandoDocente($id_usuario, $fecha, $p1, $p2, $p3, $p4, $p5, $p6, $p7, $p8, $p9, $p10, $p11, $p12, $p13, $p14, $p15, $p16, $p17, $p18, $p19, $p20, $p21, $p22, $p23, $p24, $p25, $p26, $p27, $p28, $p29, $p30, $p31, $p32, $p33, $p34,$p35,$p36);
				if ($rspta) {
					$inserto = array(
						"estatus" => 1,
	...
	case 'traer_estado_modal_docente':
		$respuesta = $paneldocente->estadoModalMonitorandoDocente($id_usuario);
	...
	case 'listarCursosEC':
		...
		$cursos = $paneldocente->listarCursosEC($fecha);
		$data = array(
			'exito' => 0,
			'info' => '',
			'thumbs' => '',
		...
		for ($i = 0; $i < count($cursos); $i++) {
			...
			$fecha_inicio = $paneldocente->convertirFechaAtexto($cursos[$i]["fecha_inicio"]);
			$create_dt = $paneldocente->convertirFechaAtexto($fecha_creacion[0]);
			$data['exito'] = 1;
	...
	case 'verificarFotoPerfil':
		require_once "../modelos/PanelDocente.php";
		$panel = new PanelDocente();
		$id_usuario = $_SESSION['id_usuario'];
		$sin_foto = $panel->docenteSinFoto($id_usuario);
	...
	case 'ingresosCampus':
			...
			<div class="row">
				...
				for ($i = 0; $i < 7; $i++) {
					...
					if ($fecha > $hoy) {
						...
						// Día actual o pasado
						$ingresos = $paneldocente->ingresoDia($id_usuario, $fecha);
		...
		if($racha==7){// si ingresa los 7 dias de la semana
            ...
            $puntos_cantidad=5;
            $validarpuntos=$paneldocente->validarpuntos($punto_nombre,$fecha,$id_usuario);// para validar si el punto de perfil fue otorgado
            if ($validarpuntos) {
                ...
                $insertarpunto=$paneldocente->insertarPunto($id_usuario,$punto_nombre,$puntos_cantidad,$fecha,$hora,$periodo_actual);

                $totalpuntos=$paneldocente->verpuntos($id_usuario);
                $puntoscredencial=$totalpuntos["puntos"];
                $sumapuntos=$puntos_cantidad+$puntoscredencial;
                $paneldocente->actulizarValor($id_usuario,$sumapuntos);
