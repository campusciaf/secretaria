...
switch ($_GET['op']) {
    ...
    case 'agregarMensajeTodos':
        ...
        for ($i = 0; $i < count($rsta); $i++) {
            if (
                $rsta[$i]["consecutivo"] != @$rsta[($i - 1)]["consecutivo"]
            ) {
    ...
    case 'listarCuotasAVencer': //Listar todos los financiados que esten a 3 dias de cumplir su cuota
        ...
        for ($i = 0; $i < count($rsta); $i++) {
            ...
            $array[] = array(
                ...
                "1" => '<div style="width:180px">'.$rsta[$i]["nombres"].'</div>',
                "2" => '<div style="width:180px">'.$rsta[$i]["apellidos"].'</div>',
                "3" => $rsta[$i]["consecutivo"],
                "4" => $consulta->formatoDinero($rsta[$i]["valor_cuota"]),
                "5" => '<div style="width:280px">'.$consulta->fechaesp($rsta[$i]["plazo_pago"]).'</div>',
                "6" => '<button class="btn bg-orange btn-block text-white btn-sm" title="Enviar correo" onclick="enviarMail(' . $rsta[$i]["id_persona"] . ')"><i class="fa-solid fa-share text-white"></i></button>',
                "7" => '<div style="width:240px">'.$select.'</div>',
                "8" => '<div style="width:240px">'.$fecha.'</div>',
                "9" => $formato_12_horas,
            );
        }
    ...
    case "verSeguimientos":
        ...
        if (empty($id_credencial)) {
            die(json_encode(array("exito" => 0, "info" => "Solicitante obligatorio")));
        }
    ...
    case 'guardarTarea': //Guarda la tarea
