...
require_once "../modelos/HvFunHabilidadAptitud.php";

$habilidadAptitud = new HvFunHabilidadAptitud();
...
switch ($_GET['op']) {
    case 'guardaryeditarhabilidad':
        if (empty($id_habilidad)) {
            $rspta = $habilidadAptitud->cv_insertarHabilidad($id_cvadministrativos, $nombre_categoria, $nivel_habilidad);
            if ($rspta) {

                $conteo = $habilidadAptitud->CuentoRegistros($id_cvadministrativos);
                $totalRegistros = isset($conteo['total']) ? intval($conteo['total']) : 0;
                if ($totalRegistros >= 2 && $porcentaje_actual < 44) {
                    // Aumenta a 20% si tiene 2 o más registros y aún no llega
                    $nuevo_porcentaje = 44;
                    $habilidadAptitud->actualizar_porcentaje_personal($nuevo_porcentaje, $id_cvadministrativos);
                } 
                $inserto = array(
                    "estatus" => 1,
            ...
            $rspta = $habilidadAptitud->cv_editarHabilidad($id_cvadministrativos, $id_habilidad, $nombre_categoria, $nivel_habilidad);
            if ($rspta) {
                $conteo = $habilidadAptitud->CuentoRegistros($id_cvadministrativos);
                $totalRegistros = isset($conteo['total']) ? intval($conteo['total']) : 0;
    ...
    case 'mostrarHabilidades':
        $habilidad_stmt = $habilidadAptitud->cv_listarHabilidad($id_cvadministrativos);
