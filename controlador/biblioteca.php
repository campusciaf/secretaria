...
switch($op){
	case 'agregar':
        ...
        /*echo("$url, $titulo, $fechaLanz, $autor, $categoria, $programa, $editorial, $isbn, $idioma, $paginas, $formato, $descripcion, $palabclave, $ejemplares");*/
		if(move_uploaded_file($_FILES['file_url']['tmp_name'], $img1path)){
			$url = $_FILES['file_url']['name'];
			$agregar_libro = $biblioteca->agregarLibro($url, $titulo, $fechaLanz, $autor, $categoria, $programa, $editorial, $isbn, $idioma, $paginas, $formato, $descripcion, $palabclave, $ejemplares);
            echo json_encode($agregar_libro);
		}
    break;
	case 'mostrar':
		$data[0] = "";
		$respuesta = $biblioteca->mostrarLibros();
		for($i = 0;$i < count($respuesta);$i++ ) {
			$data[0].='<div class="col-xs-6 col-sm-6 col-md-2 col-lg-2" style="margin:10px 0px; padding:0px">
	...
	case 'cargarFormulario':
		...
		$cargar_modal = $biblioteca->cargarFormModal($id_libro_seleccionado);
	...
	case 'modificar':
		...
		if ($file_edit == "") {
			...
			$modificar_libro = $biblioteca->modificarLibro($id_libro_modificar, $portada, $titulo, $fechaLanz, $autor, $categoria, $programa, $editorial, $isbn, $idioma, $paginas, $formato, $descripcion, $palabclave, $ejemplares);
		}else {
			$target_path = '../public/img/biblioteca/';
        	$img1path = $target_path . $_FILES['file_url']['name'];
            if(move_uploaded_file($_FILES['file_url']['tmp_name'], $img1path))
            {
                ...
                $modificar_libro = $biblioteca->modificarLibro($id_libro_modificar, $nombre_imagen, $titulo, $fechaLanz, $autor, $categoria, $programa, $editorial, $isbn, $idioma, $paginas, $formato, $descripcion, $palabclave, $ejemplares);
            }
		}
	...
	case 'eliminar':
		...
		if (isset($id_libro_eliminar)) {
			$eliminar_libro = $biblioteca->eliminarLibro($id_libro_eliminar);
	...
	case 'buscar':
		...
		$data[0] = "";
		$respuesta = $biblioteca->buscarLibros($busqueda);
		for($i = 0;$i < count($respuesta);$i++ ) {
			$data[0] .='
