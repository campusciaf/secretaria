<?php
require_once "../modelos/Web_Blog.php";
$fecha_blog = date('Y-m-d');
$hora = date('H:i');

$ip = $_SERVER['REMOTE_ADDR'];
// error_reporting(1); 
$web_blog = new Web_Blog();
...
$categoria_blog = isset($_POST["categoria_blog"]) ? limpiarCadena($_POST["categoria_blog"]) : "";
...
$categoria_blog_video = isset($_POST["categoria_blog_video"]) ? limpiarCadena($_POST["categoria_blog_video"]) : "";
...
$categoria_blog_imagen = isset($_POST["categoria_blog_imagen"]) ? limpiarCadena($_POST["categoria_blog_imagen"]) : "";
...
switch ($_GET["op"]) {
	...
	case 'listarnoticias':
		$rspta	= $web_blog->mostrarnoticias();
		...
		for ($i = 0; $i < count($reg); $i++) {
			...
			$data[] = array(
				"0" => ($reg[$i]["estado"]) ? '
				  <div class="btn-group">
					<button class="tooltip-agregar btn btn-primary btn-xs" onclick="mostrar_blog(' . $id_blog . ')" title="Editar Blog" data-toggle="tooltip" data-placement="top"><i class="fas fa-pencil-alt"></i></button>   
					<button class="tooltip-agregar btn btn-danger btn-xs" onclick="eliminar_blog(' . $id_blog . ',`' . $imagen_eliminar . '`)" title="Eliminar Blog" data-toggle="tooltip" data-placement="top"><i class="far fa-trash-alt"></i></button>
					<button class="tooltip-agregar btn btn-success btn-xs" onclick="desactivar_blog(' . $id_blog . ')" title="Desactivar" data-toggle="tooltip" data-placement="top"><i class="fas fa-lock-open"></i></button>
				  </div>' :
	...
	case 'mostrar_blog':
		$rspta = $web_blog->mostrar_blog($id_blog);
	...
	case 'guardaryeditarnoticias':
		...
		if ($material_estado == 1) {
			...
			if (!file_exists($_FILES['agregar_editar_imagen_video']['tmp_name']) || !is_uploaded_file($_FILES['agregar_editar_imagen_video']['tmp_name'])) {
				...
				if ($_FILES['agregar_editar_imagen_video']['type'] == "image/webp") {
					...
					if (move_uploaded_file($_FILES['agregar_editar_imagen_video']['tmp_name'], $img1path_movil)) {
						$imagen_blog_video = $_FILES['agregar_editar_imagen_video']['name'];
			...
			$rspta = $web_blog->editarblog($imagen_iframe, $imagen_blog_video, $titulo_blog, $subtitulo_blog, $contenido_blog_editar, $id_blog, $link_noticia_imagen_editar);
			...
			$rspta = $web_blog->editarblogimagen($imagen_blog, $titulo_blog, $subtitulo_blog, $contenido_blog_editar,$id_blog, $link_noticia_imagen_editar);
			echo $rspta ? "Blog actualizado" : "Blog no se pudo actualizar";
		}
	...
	case 'eliminar_blog':
		...
		$eliminar_blog = $web_blog->eliminarBlog($id_blog);
	...
	case "Categoria_noticia":
		$rspta = $web_blog->noticias_categoria();
	...
	case "Categoria_video":
		...
		case 'desactivar_noticia':
			$desactivar_blog = $web_blog->desactivar_blog($id_blog);
			echo json_encode($desactivar_blog);
			break;

	case 'activar_noticia':
		$rspta = $web_blog->activar_noticia($id_blog);
		echo json_encode($rspta);
	...
	case 'cambiar_formato':
		$rspta = $web_blog->cambiar_formato($id_blog);
		break;
	...
	case 'agregarvideo':
		...
		if (empty($id_blog)) {
			...
			$rspta = $web_blog->insertarblogimagenyvideo($id_blog, $url_video, $agregar_imagenvideo, $titulo_blog_video, $subtitulo_blog_video, $contenido_blog_video, $fecha_blog, $link_noticia_video, $estado, $ip, $hora, $fecha_blog, $id_usuario, $extension_categoria_video);
		...
		if (!file_exists($_FILES['agregar_imagen']['tmp_name']) || !is_uploaded_file($_FILES['agregar_imagen']['tmp_name'])) {
			...
			if ($_FILES['agregar_imagen']['type'] == "image/webp") {
				...
				if (empty($id_blog)) {
					...
					$rspta = $web_blog->insertarblog($id_blog, $imagen_blog, $titulo_blog_imagen, $subtitulo_blog_imagen, $contenido_blog_imagen, $fecha_blog, $link_noticia_imagen, $estado, $ip, $hora, $fecha_blog, $id_usuario, $extension_categoria_imagen);
	...
	case 'ver_video':
		...
		$ver_video = $web_blog->mostrar_blog($id_blog);
