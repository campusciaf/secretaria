...
session_start();
require_once "../modelos/WebTransacciones.php";
$web_transacciones = new WebTransacciones();
...
switch ($_GET["op"]) {
	case 'selectPeriodo':
		$rspta = $web_transacciones->listarPeriodos();
	...
	case 'listar':
		if ($tipo_busqueda == "dia") {
			...
			$datos = $web_transacciones->listarPagosPorDia($inicio, $fin);
		} else if ($tipo_busqueda == "identificacion") {
			$datos = $web_transacciones->listarPagosPorIdentificacion($dato_busqueda);
		} else if ($tipo_busqueda == "periodo") {
			$datos = $web_transacciones->listarPagosPorPeriodo($dato_busqueda);
		}
		$data = array();
		for ($i = 0; $i < count($datos); $i++) {
			...
			$nombre_completo = $datos[$i]["nombre_completo"];
			...
			$factura_yeminus = $datos[$i]["factura_yeminus"];
			$data[] = array(
				"0" => '<div class="text-center">
				<i class="'. $color[$yeminus_ok]." ".$si_no[$yeminus_ok]. '"></i> 	
				</div>',
				"1" => '<button class="btn btn-sm btn-outline-primary" onclick="detalletransaccion(' . $id_pagos . ')"> <i class="fas fa-list"></i> ' . $id_pagos . '</button>',
				"2" => explode(" ",$x_fecha_transaccion)[0],
				"3" => $factura_yeminus,
				"4" => $x_amount_base,
				"5" => $identificacion_estudiante,
				"6" => $nombre_completo,
				"7" => $celular,
				"8" => $x_franchise,
				"9" => $x_id_factura,
				"10" => $x_description,
				"11" => $x_respuesta == "Aceptada" ? "<span style='color:#B7EC1E'><b>Aceptada</b></span>" : $x_respuesta,
			);
		}
		$results = array(
	...
	case "detalletransaccion":
		...
		$datos = $web_transacciones->listarPagosDetalle($id_pago);
		$data["0"] .= '
		<div class="row">
			<div class="col-xl-8">
				<div class="card">
