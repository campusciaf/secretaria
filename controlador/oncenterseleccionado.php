<?php
session_start();
require_once "../modelos/OncenterSeleccionado.php";
date_default_timezone_set("America/Bogota");
$fecha = date('Y-m-d');
$hora = date('H:i:s');
$oncenterseleccionado = new OncenterSeleccionado();
$rsptaperiodo = $oncenterseleccionado->periodoactual();
...
switch ($_GET["op"]) {
	...
	case 'moverUsuario':
		$rspta = $oncenterseleccionado->moverUsuario($id_estudiante_mover, $estado);
		...
		if ($rspta == "Usuario Actualizado") { // se puede insertar un  seguimiento
			...
			$rspta2 = $oncenterseleccionado->insertarSeguimiento($id_usuario, $id_estudiante_mover, $motivo_seguimiento, $mensaje_seguimiento, $fecha, $hora);
		}
		break;
	case 'listar':
		$data = array(); //Vamos a declarar un array
		$data["0"] = ""; //iniciamos el arreglo
		$consulta2 = $oncenterseleccionado->listar($periodo_campana); // consulta para traer los preinscritos de la 

		$rsptaperiodo = $oncenterseleccionado->periodoactual();	
		$periodo_campana=$rsptaperiodo["periodo_campana"];
		//resultado de la campaña actual por el periodo actual.
		$porcentajecampañaactual = $oncenterseleccionado->CantidadCasosPorCampaña($periodo_campana);
		...
		$consulta3 = $oncenterseleccionado->listarmedios(); // consulta para traer los estudiantes por medio
		for ($b = 0; $b < count($consulta3); $b++) {
			$medio = $consulta3[$b]["nombre"];
			$consulta4 = $oncenterseleccionado->listarmedioscantidad($medio, $periodo_campana); // cantidad de estudiantes por 
			$data["0"] .= '
		...
		$data["0"] .= '
			...
			<div class="col-3 d-flex align-items-center p-0 m-0" id="t-esm">
				<div class="col-12 bg-tips p-0 m-0">
					...
					<div class="card-body">
					<h2 class="titulo-2 fs-24 text-semibold">Estado Seleccionado</h2>
					<p>Espacio para gestionar los docmuentos y el pago de matricula financiera</p>
	...
	case 'listarDos':
					...
					<div class="col-6 p-4 tono-3">
						<div class="row align-items-center">
							...
							<div class="col-5 fs-14 line-height-18"> 
								<span class="">Resultados seleccionado</span> <br>
								<span class="text-semibold fs-20">Seleccionado</span> 
							</div> 
							</div>
						</div>
					</div>
		...
		$consulta1 = $oncenterseleccionado->listarJornada(); // consulta para listar las jornadas en la tabla
		for ($a = 0; $a < count($consulta1); $a++) {
			$data["0"] .= '<th>' . $consulta1[$a]["nombre"] . '</th>';
		}
		$data["0"] .= '<thead>';
		$data["0"] .= '<tbody>';
		$consulta2 = $oncenterseleccionado->listarPrograma(); // consulta para traer los programas activos
		for ($b = 0; $b < count($consulta2); $b++) {
			...
			$data["0"] .= '</td>';
			$consulta3 = $oncenterseleccionado->listarJornada(); // consulta pra listar el total por jornadas y programa
			for ($c = 0; $c < count($consulta3); $c++) {
				$jornada = $consulta3[$c]["nombre"];
				$consulta4 = $oncenterseleccionado->listarprogramajornada($nombre_programa, $jornada, $periodo);
				$data["0"] .= '<td>';
		...
		$consulta4 = $oncenterseleccionado->listarJornada(); // consulta pra listar las sumas de las columnas
		for ($d = 0; $d < count($consulta4); $d++) {
			$jornadasuma = $consulta4[$d]["nombre"];
			$consulta5 = $oncenterseleccionado->sumaporjornada($jornadasuma, $periodo);
			$data["0"] .= '<td>';
		...
		$data["0"] .= '</table>';
		$data["0"] .= '</div>';
		$consulta6 = $oncenterseleccionado->listar($periodo); // consulta para traer los interesados de la 
	...
	case 'listarTres':
		...
		$consulta1 = $oncenterseleccionado->listarJornada(); // consulta pra lñistar las jornadas en la tabla
		$num_paso = 1;
		for ($a = 0; $a < count($consulta1); $a++) {
		...
		for ($b = 0; $b < count($consulta2); $b++) {
			...
			for ($c = 0; $c < count($consulta3); $c++) {
				$jornada = $consulta3[$c]["nombre"];
				$consulta4 = $oncenterseleccionado->listarprogramamedio($nombre_programa, $jornada, $medio, $periodo);
				$data["0"] .= '<td>';
		...
		for ($d = 0; $d < count($consulta4); $d++) {
			$jornadasuma = $consulta4[$d]["nombre"];
			$consulta5 = $oncenterseleccionado->sumapormedio($jornadasuma, $medio, $periodo);
			$data["0"] .= '<td>';
		...
		$data["0"] .= '</table>';
		$data["0"] .= '</div>';
		$consulta6 = $oncenterseleccionado->listarpormedio($medio, $periodo); // consulta para traer los interesados de la 
	...
	case 'verEstudiantes':
		...
		$rspta = $oncenterseleccionado->listarprogramajornada($nombre_programa, $jornada, $periodo);
		...
		for ($i = 0; $i < count($reg); $i++) {
			...
			$data[] = array(
				...
				"4" => '<div class="fila' . $i . '">' . ($reg[$i]["documentos"] == 1 ? 'Pendiente' : '<span class="text-green">Validado</span>') . ' <a class="btn btn-link" onclick=verDocumentos(' . $reg[$i]["id_estudiante"] . ')><i class="fas fa-eye text-green"></i>Ver</a></div>',
				"5" => '<div class="fila' . $i . '">' . ($reg[$i]["matricula"] == 1 ? 'Pendiente' : '<a class="btn btn-link" onclick=verMatricula(' . $reg[$i]["id_estudiante"] . ')><i class="fas fa-eye text-green"></i>Ver</a>') . '</div>',
	...
	case 'verEstudiantesMedio':
		...
		$estado = $_GET["estado"];
		$periodo = $_GET["periodo"];
		$rspta = $oncenterseleccionado->listarprogramamedio($nombre_programa, $jornada, $medio, $periodo);
		//Vamos a declarar un array
		$data = array();
		$reg = $rspta;
		for ($i = 0; $i < count($reg); $i++) {
	...
	case 'verEstudiantesSuma':
		...
		$estado = $_GET["estado"];
		$periodo = $_GET["periodo"];
		$rspta = $oncenterseleccionado->sumaporjornada($jornada, $periodo);
		//Vamos a declarar un array
		$data = array();
		$reg = $rspta;
		for ($i = 0; $i < count($reg); $i++) {
			...
			$data[] = array(
				...
				"5" => '<div class="fila' . $i . '">' . ($reg[$i]["documentos"] == 1 ? 'Pendiente' : '<span class="text-green">Validado</span>') . ' <a class="btn btn-link" onclick=verDocumentos(' . $reg[$i]["id_estudiante"] . ')><i class="fas fa-eye text-green"></i>Ver</a></div>',
				"6" => '<div class="fila' . $i . '">' . ($reg[$i]["matricula"] == 1 ? 'Pendiente' : '<a class="btn btn-link" onclick=verMatricula(' . $reg[$i]["id_estudiante"] . ')><i class="fas fa-eye text-green"></i>Ver</a>') . '</div>',
	...
	case 'verEstudiantesSumaMedio':
		...
		$medio = $_GET["medio"];
		$periodo = $_GET["periodo"];
		$rspta = $oncenterseleccionado->sumapormedio($jornada, $medio, $periodo);
		//Vamos a declarar un array
		$data = array();
		$reg = $rspta;
		for ($i = 0; $i < count($reg); $i++) {
	...
	case 'verEstudiantesTotal':
		$periodo = $_GET["periodo"];
		$rspta = $oncenterseleccionado->listar($periodo);
		//Vamos a declarar un array
		$data = array();
		$reg = $rspta;
		for ($i = 0; $i < count($reg); $i++) {
	...
	case 'verEstudiantesTotalMedio':
		$medio = $_GET["medio"];
		$periodo = $_GET["periodo"];
		$rspta = $oncenterseleccionado->listarpormedio($medio, $periodo);
		//Vamos a declarar un array
		$data = array();
		$reg = $rspta;
		for ($i = 0; $i < count($reg); $i++) {
	...
	case "selectPeriodo":
		$data = array(); //Vamos a declarar un array
		$data["0"] = ""; //iniciamos el arreglo
		$rspta = $oncenterseleccionado->selectPeriodo();
	...
	case 'verHistorial':
		...
		$data = array(); //Vamos a declarar un array
		$data["0"] = ""; //iniciamos el arreglo
		$consulta1 = $oncenterseleccionado->verHistorial($id_estudiante); // consulta para traer los interesados
                        ...
                        <div class="card-body tono-3">
                            <div class="row">
                                <div class="col-xl-6">
                                    ...
                                    <dd>' . $oncenterseleccionado->fechaesp($fecha_ingreso) . ' a las ' . $hora_ingreso . ' del ' . $periodo_ingreso . '</dd>
                                    <dt>Medio</dt>
                                    <dd>' . $medio . '</dd>
                                </div>
                                    <div class="col-xl-6">							
	...
	case 'verHistorialTabla':
		...
		$rspta = $oncenterseleccionado->verHistorialTabla($id_estudiante);
		//Vamos a declarar un array
		$data = array();
		$reg = $rspta;
		for ($i = 0; $i < count($reg); $i++) {
			$datoasesor = $oncenterseleccionado->datosAsesor($reg[$i]["id_usuario"]);
			...
			$data[] = array(
				...
				"3" => $oncenterseleccionado->fechaesp($reg[$i]["fecha_seguimiento"]) . ' a las ' . $reg[$i]["hora_seguimiento"],
				"4" => $nombre_usuario
			);
		}
		$results = array(
	...
	case 'verHistorialTablaTareas':
		$id_estudiante = $_GET["id_estudiante"];
		$rspta = $oncenterseleccionado->verHistorialTablaTareas($id_estudiante);
		//Vamos a declarar un array
		$data = array();
		$reg = $rspta;
		for ($i = 0; $i < count($reg); $i++) {
			...
			$data[] = array(
				...
				"3" => $oncenterseleccionado->fechaesp($reg[$i]["fecha_programada"]) . ' a las ' . $reg[$i]["hora_programada"],
				"4" => $nombre_usuario
			);
		}
		$results = array(
	...
	case "selectModalidadCampana":
		$rspta = $oncenterseleccionado->selectModalidadCampana();
		...
		break;
	case "selectEstado":
		$rspta = $oncenterseleccionado->selectEstado();
		for ($i = 0; $i < count($rspta); $i++) {
	...
	//case 'eliminar':
		//$id_estudiante = $_POST["id_estudiante"];
		//$rspta1 = $oncenterseleccionado->eliminarDatos($id_estudiante);
		//$rspta2 = $oncenterseleccionado->eliminarSeguimiento($id_estudiante);
		//$rspta3 = $oncenterseleccionado->eliminarTareas($id_estudiante);
		//$rspta = $oncenterseleccionado->eliminar($id_estudiante);
		//if ($rspta == 0) {
			//echo "1";
			//$estado = "Interesado";
			//$rspta4 = $oncenterseleccionado->insertarEliminar($id_estudiante, $estado, $fecha, $hora, $id_usuario);
		//} else {
			//echo "0";
		//}
		//break;
	...
	case "selectPrograma":
		$rspta = $oncenterseleccionado->selectPrograma();
		for ($i = 0; $i < count($rspta); $i++) {
			echo "<option value='" . $rspta[$i]["nombre"] . "'>" . $rspta[$i]["nombre"] . "</option>";
		}
		break;
	case "selectJornada":
		$rspta = $oncenterseleccionado->selectJornada();
		for ($i = 0; $i < count($rspta); $i++) {
			echo "<option value='" . $rspta[$i]["nombre"] . "'>" . $rspta[$i]["nombre"] . "</option>";
		}
		break;
	case "selectTipoDocumento":
		$rspta = $oncenterseleccionado->selectTipoDocumento();
		for ($i = 0; $i < count($rspta); $i++) {
			echo "<option value='" . $rspta[$i]["nombre"] . "'>" . $rspta[$i]["nombre"] . "</option>";
		}
		break;
	case "selectNivelEscolaridad":
		$rspta = $oncenterseleccionado->selectNivelEscolaridad();
		for ($i = 0; $i < count($rspta); $i++) {
			echo "<option value='" . $rspta[$i]["nombre"] . "'>" . $rspta[$i]["nombre"] . "</option>";
		}
		break;
	case 'perfilEstudiante':
		$id_estudiante = $_POST["id_estudiante"];
		$rspta = $oncenterseleccionado->perfilEstudiante($id_estudiante);
	...
	case 'verDocumentos':
		...
		$data = array(); //Vamos a declarar un array
		$data["0"] = ""; //iniciamos el arreglo
		$consultacedula = $oncenterseleccionado->verCedula($id_estudiante); // consulta para traer el soporte de la cedula 
		$archivo_cedula = $consultacedula["nombre_archivo"];
		$data["0"] .= 'Soporte Cédula<br>';
		$data["0"] .= $consultacedula ? "<a href='../files/oncenter/img_cedula/$archivo_cedula' target='_blank' class='btn btn-success btn-block'>Ver Archivo</a>" : "<div class='alert alert-warning'>Pendiente por subir</div>";
		// ************************* //
		$consultadiploma = $oncenterseleccionado->verDiploma($id_estudiante); // consulta para traer el soporte del diploma
		$archivo_diploma = $consultadiploma["nombre_archivo"];
		$data["0"] .= '<br>Soporte Diploma Bachiller<br>';
		$data["0"] .= $consultadiploma ? "<a href='../files/oncenter/img_diploma/$archivo_diploma' target='_blank' class='btn btn-success btn-block'>Ver Archivo</a>" : "<div class='alert alert-warning'>Pendiente por subir</div>";
		// ************************* //
		$consultaacta = $oncenterseleccionado->verActa($id_estudiante); // consulta para traer el soporte del acta
		$archivo_acta = $consultaacta["nombre_archivo"];
		$data["0"] .= '<br>Soporte Acta Bachiller<br>';
		$data["0"] .= $consultaacta ? "<a href='../files/oncenter/img_acta/$archivo_acta' target='_blank' class='btn btn-success btn-block'>Ver Archivo</a>" : "<div class='alert alert-warning'>Pendiente por subir</div>";
		// ************************* //
		$consultasalud = $oncenterseleccionado->verSalud($id_estudiante); // consulta para traer el soporte de la salud
		$archivo_salud = $consultasalud["nombre_archivo"];
		$data["0"] .= '<br>Soporte Salud <br>';
		$data["0"] .= $consultasalud ? "<a href='../files/oncenter/img_salud/$archivo_salud' target='_blank' class='btn btn-success btn-block'>Ver Archivo</a>" : "<div class='alert alert-warning'>Pendiente por subir</div>";
		// ************************* //
		$consultaprueba = $oncenterseleccionado->verPrueba($id_estudiante); // consulta para traer el soporte de las pruebas saber
		$archivo_prueba = $consultaprueba["nombre_archivo"];
		$data["0"] .= '<br>Soporte Prueba saber 11 <br>';
		$data["0"] .= $consultaprueba ? "<a href='../files/oncenter/img_prueba/$archivo_prueba' target='_blank' class='btn btn-success btn-block'>Ver Archivo</a>" : "<div class='alert alert-warning'>Pendiente por subir</div>";
		// ************************* //
		$consultacompromiso = $oncenterseleccionado->verCompromiso($id_estudiante); // consulta para traer el soporte de las pruebas saber
		$archivo_compromiso = $consultacompromiso["nombre_archivo"];
		$data["0"] .= '<br>Soporte Compromiso <br>';
		$data["0"] .= $consultacompromiso ? "<a href='../files/oncenter/img_compromiso/$archivo_compromiso' target='_blank' class='btn btn-success btn-block'>Ver Archivo</a>" : "<div class='alert alert-warning'>Pendiente por subir</div>";
		// ************************* //
		$consultaproteccion = $oncenterseleccionado->verProteccion($id_estudiante); // consulta para traer el soporte de las pruebas saber
		$archivo_proteccion = $consultaproteccion["nombre_archivo"];
		$data["0"] .= '<br>Soporte proteccion <br>';
		$data["0"] .= $consultaproteccion ? "<a href='../files/oncenter/img_proteccion_datos/$archivo_proteccion' target='_blank' class='btn btn-success btn-block'>Ver Archivo</a>" : "<div class='alert alert-warning'>Pendiente por subir</div>";
		// ************************* //
		$results = array($data);
		echo json_encode($results);
		break;
