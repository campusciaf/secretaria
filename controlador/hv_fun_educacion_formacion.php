...
switch ($_GET['op']) {
    ...
    case 'guardaryeditareducacion':
        ...
        if (empty($id_formacion)) {
            $rspta = $educacionFormacion->insertarEducacion($id_cvadministrativos, $institucion_academica, $titulo_obtenido, $desde_cuando_f, $hasta_cuando_f, $mas_detalles_f, "imagen", $nivel_formacion);
            if ($rspta) {
                // contamos los registros para ver si puede el usuario continuar al siguiente paso
            ...
            $rspta = $educacionFormacion->editarEducacion($id_cvadministrativos, $institucion_academica, $titulo_obtenido, $desde_cuando_f, $hasta_cuando_f, $mas_detalles_f, $id_formacion, "imagen", $nivel_formacion);
            if ($rspta) {
                $conteo = $educacionFormacion->CuentoRegistros($id_cvadministrativos);
                $totalRegistros = isset($conteo['total']) ? intval($conteo['total']) : 0;
                ...
                // else if ($totalRegistros < 2 && $porcentaje_actual >= 22) {
                //     // Baja si tiene menos de 2 registros y el porcentaje es 22% o más
                //     $nuevo_porcentaje = 10; // o el valor correcto que quieras
                //     $educacionFormacion->actualizar_porcentaje_personal($nuevo_porcentaje, $id_cvadministrativos);
                // }
                ...
                if ($file_type == "application/pdf") {
                    $img1path = $carpeta_destino . "certificado_U" . $id_cvadministrativos . "_F" . $id_formacion . ".pdf";
                    $url_imagen = $url_imagen . "certificado_U" . $id_cvadministrativos . "_F" . $id_formacion . ".pdf";
                } else {
                    $img1path = $carpeta_destino . "certificado_U" . $id_cvadministrativos . "_F" . $id_formacion . ".jpg";
                    $url_imagen = $url_imagen . "certificado_U" . $id_cvadministrativos . "_F" . $id_formacion . ".jpg";
                }
                if ($file_type == "") {
                    $inserto = array(
                        "estatus" => 1,
    ...
    case 'mostrarEducacion':
        ...
        // $id_cvadministrativos = $rspta_usuario["id_cvadministrativos"];
        $educacions_stmt = $educacionFormacion->cv_listareducacion($id_cvadministrativos);
    ...
    case 'validarEducacionInicial':
        $id_cvadministrativos = $_POST["id_cvadministrativos"];
        $conteo = $educacionFormacion->CuentoRegistros($id_cvadministrativos);
        $totalRegistros = isset($conteo['total']) ? intval($conteo['total']) : 0;
        // obtenemos el valor del porcentaje actual.
        $resultado = $educacionFormacion->obtenerPorcentajeAvance($id_cvadministrativos);
        $porcentaje_actual = $resultado['porcentaje_avance'];
        $nuevo_porcentaje = $porcentaje_actual;
        if ($totalRegistros >= 2 && $porcentaje_actual < 22) {
            $nuevo_porcentaje = 22;
            $educacionFormacion->actualizar_porcentaje_personal($nuevo_porcentaje, $id_cvadministrativos);
    ...
    case 'actualizar_porcentaje_continuar':
        $id_cvadministrativos = $_POST["id_cvadministrativos_global"];
        $porcentaje_paso_actual = $_POST["porcentaje"];
        // obtenemos el valor del porcentaje actual.
        $obtener_porcentaje_actual = $educacionFormacion->obtenerPorcentajeAvance_dinamico($id_cvadministrativos);
        ...
        if ($porcentaje_paso_actual > $porcentaje_actual_guardado) {
            ...
            $rspta = $educacionFormacion->actualizar_porcentaje_personal_dinamico($porcentaje_paso_actual, $id_cvadministrativos);
            if ($rspta) {
