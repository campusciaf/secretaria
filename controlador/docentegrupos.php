...
require_once "../modelos/DocenteGrupos.php";
...
$docentegrupos = new DocenteGrupos();
...
$area_responsable = isset($_POST["area_responsable"]) ? limpiarCadena($_POST["area_responsable"]) : "";
$influencer_nivel_accion = isset($_POST["influencer_nivel_accion"]) ? limpiarCadena($_POST["influencer_nivel_accion"]) : "";
$influencer_dimension = isset($_POST["influencer_dimension"]) ? limpiarCadena($_POST["influencer_dimension"]) : "";
switch ($_GET["op"]) {
	...
	case 'listargrupos':
		...
		for ($i = 0; $i < count($reg); $i++) {
			...
			if ($tienepeaasignaturaresultado == 'si') {
				...
				$tienepeadocente = $docentegrupos->tienepeadocente($reg[$i]["id_docente_grupo"], $periodo_actual); // consulta para saber si el docente ya activo el PEA
			...
			$data[] = array(
				...
				"6" => '<div class="btn-group">
							<a onclick="listar(' . $id_docente_grupo . ')" class="badge badge-success pointer text-white" title="Ver listado de grupo" id="b-paso8">Listar Grupo</a>
							' . $pea . '
						</div>'
			);
		}
	...
	case 'listar':
		...
		$data["datos1"] .= '
			<div class="col-xl-auto col-lg-auto col-md-auto col-12 tono-3 py-2 m-xl-2 m-lg-2 m-md-2 m-1 pointer" onClick="enviarCorreos(`' . $ciclo . '`, `' . $jornada . '`, `' . trim($id_programa) . '`, `' . $grupo . '`, '.$id_docente_grupo.')" class="" data-toggle="modal" data-target="#modalEmail" id="t2-paso13">
			...
			<div class="col-xl-auto col-lg-auto col-md-auto col-12 tono-3 py-2 m-xl-2 m-lg-2 m-md-2 m-1 pointer" onClick="consultaContacto(`' . $ciclo . '`,`' . $jornada . '`,`' . trim($id_programa) . '`,`' . $grupo . '`,`' . $materia . '`, '.$id_docente_grupo.')" id="t2-paso14"> 
			...
			<div class="col-xl-auto col-lg-auto col-md-auto col-12 tono-3 py-2 m-xl-2 m-lg-2 m-md-2 m-1 pointer" onclick="consultaReporteFinal(' . $id_docente . ', ' . $ciclo . ', `' . $materia . '`, `' . $jornada . '`, ' . trim($id_programa) . ', ' . $grupo . ', ' . $semestre . ', '. $id_docente_grupo.')" data-toggle="modal" data-target="#modalReporteFinal" id="t2-paso15">
				<div class="row align-items-center d-inline-flex">
					<div class="pl-2">
		...
		$data["datos1"] .= '<div class="col-12" id="t2-paso16">
			<a onClick="volver()" id="volver" class="btn btn-info text-white py-2" title="Volver atrás" ><i class="fas fa-arrow-circle-left" ></i> Volver</a>
			<button type="button" onclick="reporteInfluencerGrupal(' . $id_docente_grupo . ')" id="reporteInfluencerGrupal" class="btn btn-outline-secondary py-2" title="Generar Reporte Grupal" ><i class="fas fa-plus"></i> <i class="fas fa-users"></i> </button>
		</div>
		';
		$data["datos1"] .= '</div>';
		$data["datos1"] .= '
		...
		for ($i = 0; $i < count($reg); $i++) {
			...
			$btn_influencer_mas = ($_SESSION["influencer_mas"]) ? '<a onclick=reporteInfluencer("' . $reg[$i]["id_estudiante"] . '","' . $id_docente . '","' . $reg[$i]["programa"]  . '","' . $id_materia . '") class="btn bg-danger btn-xs text-white"  title="Influencer +"> <i class="fa-solid fa-hand-holding-heart"></i> </a>' : '';
			...
			if ($rspta3["estado"] == 1) {
				...
				$estado = "<a cl
				ass='btn btn-danger btn-sm text-white' title='Esta activo'>No</a>";
			}
			...
			$data["datos1"] .= '<tr>
				...
				. $caracterizado . ' '
				. $boton_psicologia . ' 
						<a onclick=horarioEstudiante("' . $reg[$i]["id_estudiante"] . '","' . $reg[$i]["ciclo_estudiante"] . '","' . $reg[$i]["programa"] . '","' . $reg[$i]["grupo"] . '") title="Ver horario de clase" class="btn btn-primary btn-xs text-white" id="t2-paso4"> <i class="fa-solid fa-calendar fa-1x"></i> </a>'
				. $btn_influencer_mas . '
					</div>
				</td>
			...
			if ($numero_hoy == $dia_de_la_clase) {
				$boton_asistio = "";
				$asistio = $docentegrupos->consultaAsistencia($id_credencial, $id_docente_grupo, $ciclo, $reg[$i]["id_estudiante"], $id_programa, $reg[$i]["id_materia"]);
				if (count($asistio) > 0) {
					$data["datos1"] .= '<td class="text-center ancho-sm "> <span class="badge bg-success"> <i class="fas fa-check"></i> </span> </td>';
				} else {
					$no_asistio = $docentegrupos->consultaInasistenciaHoy($ciclo, $reg[$i]["id_estudiante"], $id_programa, $reg[$i]["id_materia"]);
					if (count($no_asistio) > 0) {
						$data["datos1"] .= '<td class="text-center ancho-sm "> <span class="badge bg-danger"> <i class="fas fa-times"></i> </span> </td>';
					} else {
						$data["datos1"] .= '<td class="text-center ancho-sm ">
							...
							<div class="btn-group seleccion-falta_' . $rspta4["id_credencial"] . '">
								<button type="button" id="t2-paso18" onclick="marcarAsistio(' . $rspta4["id_credencial"] . ',' . $id_docente_grupo . ',' . $ciclo . ',' . $reg[$i]["id_estudiante"] . ',' . $id_programa . ',' . $reg[$i]["id_materia"] . ')" class="marcarAsistio' . $rspta4["id_credencial"] . ' tooltips btn btn-sm btn-success pointer" title="Marcar Que Asistió"> <i class="fas fa-check"></i> </button>' . '
	...
	case 'consultaContacto':
		...
		// Iteramos por cada uno de los estudiantes dependiendo de
		for ($x = 0; $x < count($registros_especiales); $x++) {
			$listado[]["id_estudiante"] = $registros_especiales[$x]["id_estudiante"];
		}
		//Vamos a declarar un array
		$data = array();
	...
	case 'consultaReporteFinal':
		...
		if ($listado_homologados == NULL) {
			$reg = array_merge($listado_normales);
		} else {
			$reg = array_merge($listado_normales, $listado_homologados);
		}
		//trae los estudioantes de otros programas agregados al grupo
		$registros_especiales = $docentegrupos->listarEspeciales($id_docente_grupo);
		// Iteramos por cada uno de los estudiantes dependiendo de
	...
	case 'enviarCorreos':
		...
		for ($i = 0; $i < count($id); $i++) {
			...
			$array_correo[] =$correo['credencial_login'];
		}
		//trae los estudioantes de otros programas agregados al grupo
		$registros_especiales = $docentegrupos->listarEspeciales($id_docente_grupo);
		// Iteramos por cada uno de los estudiantes dependiendo de
		for ($x = 0; $x < count($registros_especiales); $x++) {
			$array_correo[] = $registros_especiales[$x]['credencial_login'];
		}
		//crear string separado por comas 
		$lista_correos = implode(", ", $array_correo);
		enviar_correo($lista_correos, $asunto_estudiantes, $template_mensaje);
		$data['result'] = "ok";
		echo json_encode($data);
		break;
	case 'nota':
	...
	case 'activarPea':
		...
		$id_pea = $_POST["id_pea"];
		$resutlado = "";
		$activarpea = $docentegrupos->activarpea($id_pea, $id_docente_grupo, $fecha, $hora, $periodo_actual);
	...
	case 'reporteinfluencer':
		...
		$celular_director = $traerdatosusuario["usuario_celular"];
		$nombre_director = $traerdatosusuario["usuario_nombre"] . ' ' . $traerdatosusuario["usuario_apellido"];
		...
		$nombre_estudiante = trim($traerdatosestuidantes["credencial_nombre"] . ' ' . $traerdatosestuidantes["credencial_nombre_2"] . ' ' . $traerdatosestuidantes["credencial_apellido"]  . ' ' . $traerdatosestuidantes["credencial_apellido_2"]);
		...
		$rspta = $docentegrupos->insertarreporteinfluencer($id_estudiante_in, $id_docente_in, $id_programa_in, $id_materia_in, $influencer_mensaje, $fecha, $hora, $periodo_actual, $influencer_nivel_accion, $influencer_dimension, $area_responsable);
		if ($rspta){
			enviar_correo($correo.", sistemasdeinformacion@ciaf.edu.co", $asunto, $mensaje);
			if($influencer_nivel_accion == "Alta"){
	...
	case 'marcarAsistio':
		...
		$id_materia = $_POST['id_materia'];
		$fecha = date("Y-m-d");
		$rspta = $docentegrupos->marcarAsistio($id_credencial, $id_docente_grupo, $ciclo, $id_estudiante, $id_programa, $id_materia, $fecha);
		if ($rspta) {
			$data = array("exito" => 1, "info" => "Asistencia registrada correctamente");
		} else {
			$data = array("exito" => 0, "info" => "Error al registrar la asistencia");
		}
		echo json_encode($data);
		break;
	case 'iniciarhorario':
		...
		for ($i = 0; $i < count($reg); $i++) {
			...
			if ($tienepeaasignaturaresultado == 'si') {
				...
				$tienepeadocenteresultado = $tienepeadocente ? 'si' : 'no';
				if ($tienepeadocenteresultado == "si") {
					$pea = '<div class="bg-primary"><a href="peadocente.php?id=' . $reg[$i]["id_docente_grupo"] . '" class="bg-primary pointer text-white" id="b-paso9">Gestión PEA </a></div>';
					$billeteraasig = '<img src="../public/img/coin.webp" alt="coin" class="img-fluid" > ' . $tienepeadocente["billetera_asignatura"] . ' Pts.';
				} else {
					$pea = '<a onclick="activarPea(' . $reg[$i]["id_docente_grupo"] . ',' . $id_pea . ')" class="badge badge-danger pointer text-white" title="Activar el plan educativo de aula" id="b-paso9">Activar PEA </a>';
					$billeteraasig = "";
				}
			} else {
				$pea = "<div class='text-danger'>Sin PEA</div>"; // o tiene pea desde la dirección
				$billeteraasig = "";
			}
			$listargrupo = '<a onclick="listar(' . $id_docente_grupo . ')" class="pointer text-white" title="Ver listado de grupo" id="b-paso8">Estudiantes</a>';
			...
			$impresion .= json_encode([
				...
				"billetera" => $billeteraasig,
			]);
		}
		$impresion .= ']';
		echo $impresion;
		break;
	case 'listarReportesEstudiante':
		...
		$id_estudiante = $_POST['id_estudiante'];
		$id_docente = $_POST['id_docente'];
		$rspta = $docentegrupos->listarReportesEstudiante($id_estudiante, $id_docente);
		$data["info"] = "";
		for ($i = 0; $i < count($rspta); $i++) {
			$data["info"] .=
				"<tr>" .
				"<td><button onclick='mostrarInfoReporte(" . $rspta[$i]['id_influencer_reporte'] . ")' class='btn btn-primary btn-xs' title='Ver reporte'><i class='fa fa-eye'></i></button></td>" .
				"<td> <i class='fas fa-circle " . $color_nivel_accion[$rspta[$i]['influencer_nivel_accion']] . "'> </i> " . $texto_nivel_accion[$rspta[$i]['influencer_nivel_accion']] . "  </td>" .
				"<td>" . $rspta[$i]['influencer_dimension'] . "</td>" .
				"<td>" . $rspta[$i]['influencer_mensaje'] . "</td>" .
				"<td>" . $estado[$rspta[$i]['reporte_estado']] . "</td>" .
				"<td>" . $rspta[$i]['fecha'] . "</td>" .
				"<td>" . $rspta[$i]['hora'] . "</td>" .
				"<td>" . $rspta[$i]['periodo'] . "</td>
			</tr>";
		}
		echo json_encode($data);
		break;
	case 'mostrarInfoReporte':
		...
		$rspta = $docentegrupos->mostrarInfoReporte($id_influencer_reporte);
		$html = "";
		if (isset($rspta["docente_nombre"])) {
			$data["exito"] = 1;
			$fecha = $docentegrupos->fechaesp($rspta["fecha"]);
			...
			$html .= '    </div>';
			$html .= '</div>';
			$rspta_reporte = $docentegrupos->respuestasReporteInfluencer($id_influencer_reporte);
			for ($i = 0; $i < count($rspta_reporte); $i++) {
				$fecha = explode(" ", $rspta_reporte[$i]["created_dt"]);
				$fecha_respuesta = $docentegrupos->fechaesp($fecha[0]);
	...
	case 'cerrarReporteInfluencer':
		...
		$cierre_caso = $docentegrupos->cerrarReporteInfluencer($influencer_tipo_cierre, $influencer_reflexion, $influencer_acciones, $influencer_resultado_final, $influencer_comentario_final, $id_influencer_reporte);
		if ($cierre_caso) {
		...
		echo json_encode($data);
		break;
	case 'listarReportesGrupo':
		$id_docente_grupo = $_POST['id_docente_grupo'];
		$id_docente = $_SESSION['id_usuario'];
		$rspta = $docentegrupos->listarReportesGrupo($id_docente_grupo, $id_docente);
		$data["info"] = "";
		for ($i = 0; $i < count($rspta); $i++) {
			$data["info"] .=
				"<tr>" .
		...
		echo json_encode($data);
		break;
	case 'reporteinfluencerGrupo':
		$id_docente_grupo = $_POST["id_docente_grupo"];
		$id_docente = $_SESSION['id_usuario'];
		$rspta = $docentegrupos->insertarReporteInfluencerGrupo($id_docente_grupo, $id_docente, $influencer_mensaje, $fecha, $hora, $periodo_actual);
