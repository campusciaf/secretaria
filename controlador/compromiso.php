...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($id_compromiso)){
			$rspta=$compromiso->insertar($id_usuario,$compromiso_nombre,$compromiso_fecha,'0','0',$compromiso_valida,$compromiso_periodo);
			echo $rspta ? "compromiso registrado" : "compromiso no se pudo registrar";
		}
		else {
			$rspta=$compromiso->editar($id_compromiso,$id_usuario,$compromiso_nombre,$compromiso_fecha,$compromiso_val_admin,$compromiso_val_usuario,$compromiso_valida,$compromiso_periodo);
	...
	case 'guardaryeditarmeta':
		if (empty($id_meta)){
			$rspta=$compromiso->insertarmeta($id_compromiso_meta,$id_eje,$meta_nombre,$meta_fecha,$meta_val_admin,$meta_val_usuario,$meta_valida,$meta_periodo,$corresponsabilidad,$_POST["condicion_institucional"],$_POST["condicion_programa"]);
			echo $rspta ? "meta registrado" : "meta no se pudo registrar";
		}
		else {
			$rspta=$compromiso->editarmeta($id_meta,$id_compromiso_meta,$id_eje,$meta_nombre,$meta_fecha,$meta_val_admin,$meta_val_usuario,$meta_valida,$meta_periodo,$corresponsabilidad,$_POST["condicion_institucional"],$_POST["condicion_programa"]);
			echo $rspta ? "meta actualizada" : "meta no se pudo actualizar";
		}
	break;	

	case 'mostrar':
		$rspta=$compromiso->mostrar($id_compromiso);
 		//Codificar el resultado utilizando json
 		echo json_encode($rspta);
	break;
		
	case 'mostrarmetaeditar':
		$rspta=$compromiso->mostrarMetaEditar($id_meta);
 		//Codificar el resultado utilizando json
 		echo json_encode($rspta);
	break;	
	...
	case 'mostrarmeta':
		...
		$rspta=$compromiso->mostrarMeta($id_compromiso,$periodo_actual);
 		$data= Array();
		$data["0"] ="";
		...
		$reg=$rspta;
 		...
 		for ($i=0;$i<count($reg);$i++){	
			...
			$rsptaeje=$compromiso->buscarEjes($reg[$i]["id_eje"]);
	...
	break;
		...
		$rspta=$compromiso->listarUsuarioPoa();
		...
		$reg=$rspta;
 		...
 		for ($i=0;$i<count($reg);$i++){	
			...
			$rsptadatos=$compromiso->listarCompromiso($reg[$i]["id_usuario"],$periodo_actual);
			$regdatos=$rsptadatos;
			...
			$data["0"] .= '	
                ...
                <div class="box box-solid">
					...
					$data["0"] .= '	
									...
									$rsptameta=$compromiso->mostrarMeta($regdatos[$j]["id_compromiso"],$periodo_actual);
									

									for ($k=0;$k<count($rsptameta);$k++){
										
										$rsptaeje=$compromiso->buscarEjes($rsptameta[$k]["id_eje"]);
                                    ...
                                    <p style="margin-bottom: 0">
                                        <i class="far fa-calendar-alt"></i> '.$compromiso->fechaesp($regdatos[$j]["compromiso_fecha"]).'
                                    </p>
                                </div>
                            </div>
                        </div>
	...
	case 'eliminar':
		$rspta=$compromiso->eliminar($id_compromiso);
		
		$rspta2=$compromiso->eliminarCompromisoMeta($id_compromiso);

 		echo json_encode($rspta);
	break;
	
	case 'validarcompromiso':
		$rspta=$compromiso->validarCompromiso($id_compromiso);
 		//Codificar el resultado utilizando json
 		echo json_encode($rspta);
	break;	
	...
	case 'eliminarmeta':
		$rspta=$compromiso->eliminarMeta($id_meta);
 		//Codificar el resultado utilizando json
 		echo json_encode($rspta);
	break;
	
	case "selectEjes":	
		$rspta = $compromiso->selectEjes();
	...
	case "selectPeriodo":	
		$rspta = $compromiso->selectPeriodo();
		for ($i=0;$i<count($rspta);$i++)
				{
	...
	$rspta = $compromiso->selectListaSiNo();
		for ($i=0;$i<count($rspta);$i++)
			{
	...
	$rspta = $compromiso->selectlistarCargo();
		for ($i=0;$i<count($rspta);$i++)
			{
	...
	case 'imprimir':
		...
		$rspta=$compromiso->listarImprimir($id_usuario_reporte);
 		//Vamos a declarar un array
 		$data= Array();
		$data["0"] ="";
		$data["0"] .= '<h3>' . $rspta["usuario_cargo"] .'</h3>';
		
		$rsptadatos=$compromiso->listarCompromiso($id_usuario_reporte,$periodo_actual);
			$regdatos=$rsptadatos;
			
			for ($j=0;$j<count($regdatos);$j++){
				
				$data["0"] .= '<i class="fas fa-hands-helping"></i> <b> '.$regdatos[$j]["compromiso_nombre"].'</b><br>
				Compromiso para entregar: '.$compromiso->fechaesp($regdatos[$j]["compromiso_fecha"]).'<br><br>
				...
				$data["0"] .= '<ol>';
									for ($k=0;$k<count($rsptameta);$k++){
										...
										<b>Fecha de entrega:</b> '.$compromiso->fechaesp($rsptameta[$k]["meta_fecha"]).'<br>';
	...
	case "fechaLimite":	
	$rspta = $compromiso->fechaLimite($id_compromiso);
	...
	case 'condiciones_institucionales':
		...
		$rspta = $compromiso->listarCondicionesInstitucionales();

		//Obtener las condiciones asignadas a la meta
		$id=$_GET['id'];
		$marcados = $compromiso->listarcondicionesmarcadas($id);
...
//		Mostramos la lista de condicones-insitucionales en la vista y si están o no marcados
	...
	case 'condiciones_programa':
		...
		$rspta = $compromiso->listarCondicionesprograma();

		//Obtener las condiciones asignadas a la meta
		$id=$_GET['id'];
		$marcados = $compromiso->listarcondicionesmarcadasprograma($id);
		//Declaramos el array para almacenar todos las condiciones marcados
		$valores=array();
