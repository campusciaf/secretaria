<?php
session_start();
require_once "../modelos/HvFunDocumentosAdicionales.php";
$documentosAdicionales = new HvFunDocumentosAdicionales();
...
switch ($_GET['op']) {
    ...
    case 'guardaryeditardocumentos_adicionales':
        ...
        if (empty($id_documentacion)) {
            $rspta = $documentosAdicionales->cvinsertarDocumentosA($id_cvadministrativos, $documento_nombreA, "imagen");
            if ($rspta) {
                $conteo = $documentosAdicionales->CuentoRegistros($id_cvadministrativos);
                $totalRegistros = isset($conteo['total']) ? intval($conteo['total']) : 0;
                if ($porcentaje_actual < 88) {
                    // Aumenta a 20% si tiene 2 o más registros y aún no llega
                    $nuevo_porcentaje = 88;
                    $documentosAdicionales->actualizar_porcentaje_personal($nuevo_porcentaje, $id_cvadministrativos);
                } 
    ...
    case 'mostrarDocumentosAdicionales':
        $documentosA_stmt = $documentosAdicionales->cvalistarDocumentosAdicionales($id_cvadministrativos);
