<?php
session_start();
require_once "../modelos/CalificarHuella.php";

$calificarhuella = new CalificarHuella();
...
switch ($_GET["op"]) {
	...
	case 'guardaryeditar':
		...
		$rspta = $calificarhuella->insertar($credencial_nombre, $credencial_nombre_2, $credencial_apellido, $credencial_apellido_2, $credencial_identificacion, $credencial_login, $credencial_clave);


		$data["0"] = $rspta ? "Estudiante registrado(a) " : "No se pudo registrar el estudiante";

		$rspta = $calificarhuella->traeridcredencial($credencial_identificacion);
	...
	case 'verificardocumento':
		...
		$rspta = $calificarhuella->verificardocumento($credencial_identificacion);
	...
	case 'listar':
		...
		$rspta = $calificarhuella->listar($id_credencial);
		//Vamos a declarar un array
		$data = array();
		...
		while ($i < count($rspta)) {
			...
			$data[] = array(
				...
				<button class="btn btn-primary btn-xs" onclick="mostrarmaterias(' . $rspta[$i]["id_programa_ac"] . ',' . $rspta[$i]["id_estudiante"] . ',' . $rspta[$i]["ciclo"] . ')" title="Matricular Materias"><i class="fas fa-plus-square"></i> Materias</button>',
	...
	case 'mostrardatos':
		...
		$verificar_cedula = $calificarhuella->mostrardatos($id_credencial);
		...
		// case "mostrardatos":
		// 	$id_credencial=$_POST["id_credencial"];
		// 	$rspta = $calificarhuella->mostrardatos($id_credencial);
		// 	$data= Array();
		// 	$data["0"] ="";
	...
	case "mostrarmaterias":
		...
		$rspta2 = $calificarhuella->datosPrograma($id_programa_ac);
		$reg2 = $rspta2;
		...
		$rspta4 = $calificarhuella->datosEstudiante($id_estudiante);
		$reg4 = $rspta4;
		...
		while ($semestres <= $semestres_del_programa) {
			...
			$rspta = $calificarhuella->listarMaterias($id_estudiante, $ciclo, $semestres);
			$reg = $rspta;
			...
			for ($i = 0; $i < count($reg); $i++) {
				...
				while ($inicio_cortes <= $cortes) {
					...
					$data["0"] .= '<td class="ancho-md"><input type="number" value="' . $reg[$i][$corte_nota] . '" onchange=nota(this.value,"' . $reg2["ciclo"] . '","' . base64_encode($reg[$i]['id_materia']) . '","' . $id_programa_ac . '","' . $corte_nota . '") class="form-control"></td>';
					$inicio_cortes++;
				}
				$da = $calificarhuella->valorhuella();
				$data["0"] .= '
				<td class="ancho-md"><input type="number" value="' . $reg[$i]["promedio"] . '" onchange=promedio(this.value,"' . $reg2["ciclo"] . '","' . base64_encode($reg[$i]['id_materia']) . '") class="form-control"></td>
				<td class="ancho-xl">
				<select class="form-control" onchange=huella(this.value,"' . base64_encode($reg[$i]['id_materia']) . '","' . $reg2["ciclo"] . '")>';
	...
	// 	$rspta = $calificarhuella->mostrar($id);
	...
	case "matriculaMateriaNormal":
		...
		$rspta2 = $calificarhuella->MateriaDatos($id_materia);
		$nombre_materia = $rspta2["nombre"];
		$semestre = $rspta2["semestre"];
		$creditos = $rspta2["creditos"];

		$rspta3 = $calificarhuella->datosEstudiante($id_estudiante);
		...
		$rspta4 = $calificarhuella->insertarmateria($id_estudiante, $nombre_materia, $jornada_e, $periodo_actual, $semestre, $creditos, $id_programa_ac, $ciclo, $fecha, $usuario, $grupo);

		if ($rspta4) {
			$rspta5 = $calificarhuella->actualizarperiodo($id_estudiante, $periodo_actual);

			$rspta6 = $calificarhuella->creditosMatriculados($id_estudiante, $ciclo); //suma el total de creditos matriculados
			$creditos_matriculados = $rspta6["suma_creditos"];

			$rspta7 = $calificarhuella->datosPrograma($id_programa_ac); // trae creditos por semestre
			...
			$rspta8 = $calificarhuella->actualizarsemestre($id_estudiante, $semestre_nuevo); // trae creditos por semestre
		}
	...
	case "eliminarMateria":
		...
		$rspta9 = $calificarhuella->trazabilidadMateriaEliminada($id_estudiante, $nombre_materia, $jornada_e, $periodo_actual, $semestre, $promedio_materia_matriculada, $programa, $fecha, $usuario);

		$rspta4 = $calificarhuella->eliminarMateria($id_materia_matriculada, $ciclo);
	...
	case "selectJornada":
		$rspta = $calificarhuella->selectJornada();
	...
	case "selectPrograma":
		$rspta = $calificarhuella->selectPrograma();
		for ($i = 0; $i < count($rspta); $i++) {
	...
	case "selectPeriodo":
		$rspta = $calificarhuella->selectPeriodo();
		for ($i = 0; $i < count($rspta); $i++) {
	...
	case "selectGrupo":
		$rspta = $calificarhuella->selectGrupo();
		for ($i = 0; $i < count($rspta); $i++) {
	...
	case "actualizarJornada":
		...
		$rspta = $calificarhuella->actualizarJornada($id_materia, $jornada_e, $ciclo);
	...
	case "actualizarPeriodo":
		...
		$rspta = $calificarhuella->actualizarPeriodoMateria($id_materia, $periodo, $ciclo);
	...
	case "actualizarGrupo":
		...
		$rspta = $calificarhuella->actualizarGrupoMateria($id_materia, $grupo, $ciclo);
	...
	case 'huella':
		...
		$val = $_POST['val'];
		$c = $_POST['c'];
		$calificarhuella->huella($id, $val, $c);
		break;
	case 'nota':
		$id = $_POST['id'];
		$nota = $_POST['nota'];
		...
		$c = $_POST['c'];
		$pro = $_POST['pro'];
		$calificarhuella->agreganota($id, $nota, $tl, $c, $pro);
		break;
	...
	case 'promedio':
		...
		$val = $_POST['val'];
		$c = $_POST['c'];
		$calificarhuella->promedio($id, $val, $c);
		break;
}
