<?php 
require_once "../modelos/Oncenteradmintareas.php";
...
switch ($_GET['op']) {
    case 'consulta':
        ...
        for ($i=0; $i < count($asesores); $i++) {
            ...
            $cumplida_g=$cumplida_g+$c_cumplida+$l_cumplida+$s_cumplida;
            $hoy_g=$hoy_g+$c_hoy+$l_hoy+$s_hoy;
            $nocumplida_g=$nocumplida_g+$c_no_realizada+$l_no_realizada+$s_no_realizada;
            $total_hoy_g=$total_hoy_g+$total_hoy;
        }
        
		$data['conte'] .= '</div>';
        $data['general'] .= $total_g;
        $data['cumplidas'] .= ($cumplida_g);
        $data['pendientes'] .= ($hoy_g);
        $data['nocumplidas'] .= ($nocumplida_g);
        $data['deldia'] .= $total_hoy_g;
    ...
    case 'buscar':
        ...
        if($cslt==5){
            ...
            $datos = $consulta->buscar($id,$sql,$sql2,$fecha_tarea_tabla);
        }
                    ...
                    <div class="col-12 p-4">
                        <table class="table" id="tbl_buscar" style="width:100%">
                            ...
                            for ($i=0; $i <count($datos); $i++) {
                                ...
                                $nombre_estudiante = $datos_estudia['nombre'].' '.$datos_estudia['nombre_2'].' '.$datos_estudia['apellidos'].' '.$datos_estudia['apellidos_2'];
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a onclick="agregar('.$datos[$i]['id_estudiante'].')" class="btn btn-success btn-xs text-white" title="Agregar Seguimiento">
