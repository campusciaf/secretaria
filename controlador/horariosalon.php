<?php 
require_once "../modelos/HorarioSalon.php";

$horariosalon=new HorarioSalon();
...
$rsptaperiodo = $horariosalon->periodoactual();
...
switch ($_GET["op"]){
	...
	case 'iniciarcalendario':
		...
		$traerhorario=$horariosalon->TraerHorariocalendario($salon,$periodo);
		...
		for ($i=0;$i<count($traerhorario);$i++){
			...
			$datosmateria=$horariosalon->BuscarDatosAsignatura($id_materia);
			$nombre_materia=$datosmateria["nombre"];

			$bucarnumestudiantes=$horariosalon->buscarNumEstudiantes($ciclo,$nombre_materia,$jornada,$id_programa,$grupo,$periodo_actual);

			if($id_usuario_doc==null){
				$nombre_docente="Sin Asignar";
			}else{
				$datosdocente=$horariosalon->datosDocente($id_usuario_doc);
	...
	case "selectPeriodo":	
		$rspta = $horariosalon->selectPeriodo();
	...
	case "selectSalon":	
		$rspta = $horariosalon->selectSalon();
		echo "<option value=''>Seleccionar</option>";
		for ($i=0;$i<count($rspta);$i++)
				{
