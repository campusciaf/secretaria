<?php
require_once "../modelos/Disponibilidad_Salones_Admin.php";
// para enviar el correo en caso de que el formulario en codigo_salon sea igual a 1 
require "../mail/send_reservas_notificacion.php";
require "../mail/template_notificacion_reservas_fun.php";
...
switch ($_GET['op']) {
    ...
    case 'consultarSalones':
        ...
        $data[0] .= '
            ...
            <div class="col-12 tono-2 py-2" style="height: 500px; overflow-y: auto;">';
    ...
    case 'iniciarcalendario':
        ...
        for ($j = 0; $j < count($eventos_reservados); $j++) {
            if (count($eventos_horarios) > 1 && $j == 0) {
                $impresion .= ',';
            }
            $id_docente_r = $eventos_reservados[$j]["id_docente"];
            $nombre_materia_r = $eventos_reservados[$j]["detalle_reserva"]." ". $eventos_reservados[$j]["materia_evento"];
            ...
            $datosdocente_r = $consulta->colaborador_datos($id_docente_r);
            $nombre_docente_r = $datosdocente_r["usuario_nombre"] . ' ' . $datosdocente_r["usuario_apellido"];

            $color_r = "#ff9900";
            $impresion .= '{"title":"' . $nombre_materia_r . ' - doc: ' . $nombre_docente_r . ' ","daysOfWeek":"' . $diasemana_r . '","startTime":"' . $horainicio_r . '","endTime":"' . $horafinal_r . '","color":"' . $color_r . '", "fijo": "0"}';
    ...
    case 'aggReserva':
        ...
        // colocamos los campos para que no sean requeridos ya que los colocamos requeridos desde el jquery para los campos que ya estaban
        ...
        // estado para saber si enviamos el correo cuando es igual a 1
        ...
        if (in_array('otro', $requerimientos) && !empty($_POST['req_otro'])) {
            $requerimientos = array_diff($requerimientos, ['otro']);
            $requerimientos[] = $_POST['req_otro']; 
        }
        ...
        // si llega la palabra otro muestra lo escrito en el campo otro en caso de no estar otro muestra el campo normal.
        ...
        if ($rspta) {
            if ($estado_formulario == 1) {
                $mensaje = set_template_notificacion_reservas_fun($hora, $hasta, $codigo_salon, $nombre_docente, $correo_docente, $telefono_docente, $programa_final, $asistentes_final, $materia_evento, $experiencia_nombre, $experiencia_objetivo, $duracion_horas, $requerimientos, $requerimientos_otro, $novedad, $fecha_reserva);
