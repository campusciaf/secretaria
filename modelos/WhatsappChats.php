...
class WhatsappChats{
    public function ListarDependencias(){
        global $mbd;
        $sentencia = $mbd->prepare("SELECT DISTINCT(`dependencia`) FROM `dependencias` ORDER BY `dependencias`.`dependencia` ASC");
        $sentencia->execute();
    ...
    function listarChats() {
        global $mbd;
        $sentencia = $mbd->prepare("SELECT `ce`.`id_credencial`, `ce`.`credencial_identificacion`, `ce`.`credencial_nombre`, `ce`.`credencial_nombre_2`, `ce`.`credencial_apellido`, `ce`.`credencial_apellido_2`, `wr`.`numero_whatsapp`, `wr`.`ultima_modificacion`, `wr`.`ultimo_mensaje`, `wr`.`mensajes_no_vistos`, `wr`.`mostrado`, `wr`.`tipo_mensaje_masivo` FROM `credencial_estudiante` `ce` RIGHT JOIN `whatsapp_registros` `wr` ON `ce`.`id_credencial` = `wr`.`id_credencial` ORDER BY `wr`.`ultima_modificacion` DESC LIMIT 200;");
        $sentencia->execute();
        $registros = $sentencia->fetchAll(PDO::FETCH_ASSOC);
        return $registros;
    }
    ...
    public function ActivarSeguimiento($numero_celular, $id_usuario) {
        global $mbd; 
        // Academico = estudiante_seguimietno
        // Financiera = Sofi_seguimiento
        // Admisiones = on_seguimientos
        // Bienestar = estudiante_seguimietno
        // Egresados - seguimientos egresados
    ...
    public function insertarSeguimiento($seg_descripcion, $seg_tipo, $id_usuario, $id_credencial, $id_persona){
        global $mbd;
    ...
    public function enviarMensajeMeta($token, $url, $mensaje, $numero_celular, $datos_mensaje) {
        ...
        if ($status_code == 200) {
            ...
            // Incluir la respuesta en el mensaje de error
            return array(
                "exito" => 0,
                "err_info" => "Error al enviar el mensaje",
                "status_code" => $status_code,
                "response" => $response // Aquí está la respuesta del servidor
            );;
        }
    }
    function CantidadNoMostradosPorNumero($numero_whatsapp){
        global $mbd;
        $numero_whatsapp = substr($numero_whatsapp, 2);
