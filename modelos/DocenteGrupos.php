...
class DocenteGrupos
{
	...
	{
		$periodo_actual = $_SESSION["periodo_actual"];
		$sql = "SELECT *, $ciclo_matriculado AS `ciclo_estudiante` FROM `materias$ciclo_matriculado` WHERE `periodo` LIKE :periodo_actual AND `id_docente_grupo_esp` = :id_docente_grupo AND `activar_grupo_esp` = 1 AND `id_estudiante` = :id_estudiante ORDER BY `id_docente_grupo_esp` DESC";
		global $mbd;
		$consulta = $mbd->prepare($sql);
	...
	{
		$periodo_actual = $_SESSION["periodo_actual"];
		$sql = "SELECT `ce`.`credencial_nombre`, `ce`.`credencial_nombre_2`,`ce`.`credencial_apellido`, `ce`.`credencial_apellido_2`, `ce`.`credencial_identificacion`, `ce`.`credencial_login`, `he`.* 
		FROM ((`credencial_estudiante` `ce` INNER JOIN `estudiantes` `e` ON `e`.`id_credencial` = `ce`.`id_credencial`) 
		INNER JOIN `horario_especial` `he` ON `he`.`id_estudiante` = `e`.`id_estudiante` ) WHERE `he`.`id_docente_grupo` = :id_docente_grupo AND `he`.`periodo_especial` LIKE :periodo_actual";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":id_docente_grupo", $id_docente_grupo);
		$consulta->bindParam(":periodo_actual", $periodo_actual);
	...
	public function listar_homologados($materia, $id_docente_grupo)
	{
		...
		$sql = "SELECT * FROM materias11 INNER JOIN horario_especial ON materias11.nombre_materia = horario_especial.nombre_materia WHERE materias11.`nombre_materia` = :materia AND materias11.`periodo` = :periodo_actual AND horario_especial.id_docente_grupo = :id_docente_grupo";
		// echo $sql;
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":materia", $materia);
	...
	//Implementamos un método para insertar una falta en la tabla faltas
	public function marcarAsistio($id_credencial, $id_docente_grupo, $ciclo, $id_estudiante, $id_programa, $id_materia, $fecha_asistencia)
	{
		global $mbd;
		$periodo_asistencia = $_SESSION['periodo_actual'];
		//CREATE TABLE IF NOT EXISTS `asistencias` ( `id_asistencia` INT AUTO_INCREMENT PRIMARY KEY, `id_credencial` INT NULL, `id_estudiante` INT NULL, `id_materia` INT NULL, `id_docente_grupo` INT NULL, `fecha_asistencia` DATE NULL, `ciclo` INT NULL, `id_programa` INT NULL, `periodo_asistencia` VARCHAR(20) NULL );
		$sql = "INSERT INTO `asistencias`( `id_credencial`, `id_estudiante`, `id_materia`, `id_docente_grupo`, `fecha_asistencia`, `ciclo`, `id_programa`, `periodo_asistencia`) VALUES(:id_credencial, :id_estudiante, :id_materia, :id_docente_grupo, :fecha_asistencia, :ciclo, :id_programa, :periodo_asistencia)";
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":id_credencial", $id_credencial);
		$consulta->bindParam(":id_estudiante", $id_estudiante);
		$consulta->bindParam(":id_materia", $id_materia);
		$consulta->bindParam(":id_docente_grupo", $id_docente_grupo);
		$consulta->bindParam(":fecha_asistencia", $fecha_asistencia);
		$consulta->bindParam(":ciclo", $ciclo);
		$consulta->bindParam(":id_programa", $id_programa);
		$consulta->bindParam(":periodo_asistencia", $periodo_asistencia);
		return $consulta->execute();
	}
	function consultaAsistencia($id_credencial, $id_docente_grupo, $ciclo, $id_estudiante, $id_programa, $id_materia)
	{
		global $mbd;
		$fecha_hoy = date("Y-m-d");
		$sql = "SELECT * FROM `asistencias` WHERE `id_credencial` = :id_credencial AND `id_docente_grupo` = :id_docente_grupo AND `ciclo` = :ciclo AND `id_estudiante` = :id_estudiante AND `id_programa` = :id_programa AND `id_materia` = :id_materia AND `fecha_asistencia` = :fecha_asistencia";
		//$sql = "SELECT * FROM `asistencias` WHERE `id_credencial` = $id_credencial AND `id_docente_grupo` = $id_docente_grupo AND `ciclo` = $ciclo AND `id_estudiante` = $id_estudiante AND `id_programa` = $id_programa AND `id_materia` = $id_materia AND `fecha_asistencia` = '$fecha_hoy'";
		//echo $sql;
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":id_credencial", $id_credencial);
		$consulta->bindParam(":id_docente_grupo", $id_docente_grupo);
		...
		$consulta->bindParam(":id_programa", $id_programa);
		$consulta->bindParam(":id_materia", $id_materia);
		$consulta->bindParam(":fecha_asistencia", $fecha_hoy);
		$consulta->execute();
		$registro = $consulta->fetchAll(PDO::FETCH_ASSOC);
		return $registro;
	}
	function consultaInasistenciaHoy($ciclo, $id_estudiante, $id_programa, $id_materia)
	{
		global $mbd;
		$fecha_hoy = date("Y-m-d");
		$sql = "SELECT * FROM `faltas` WHERE `ciclo` = :ciclo AND `id_estudiante` = :id_estudiante AND `programa` = :id_programa AND `id_materia` = :id_materia AND `fecha_falta` = :fecha_falta";
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":ciclo", $ciclo);
		$consulta->bindParam(":id_estudiante", $id_estudiante);
		$consulta->bindParam(":id_programa", $id_programa);
		$consulta->bindParam(":id_materia", $id_materia);
		$consulta->bindParam(":fecha_falta", $fecha_hoy);
		$consulta->execute();
		$registro = $consulta->fetchAll(PDO::FETCH_ASSOC);
		return $registro;
	}
	...
	public function insertarreporteinfluencer($id_estudiante, $id_docente, $id_programa, $id_materia, $influencer_mensaje, $fecha, $hora, $periodo, $influencer_nivel_accion, $influencer_dimension, $area_responsable)
	{
		global $mbd;
		$sql = "INSERT INTO `influencer_reporte`(`id_estudiante`, `id_docente`, `id_programa`, `id_materia`, `influencer_mensaje`, `fecha`, `hora`, `periodo`, `influencer_nivel_accion`, `influencer_dimension`, `area_responsable` ) VALUES('$id_estudiante','$id_docente','$id_programa','$id_materia','$influencer_mensaje','$fecha','$hora','$periodo', '$influencer_nivel_accion', '$influencer_dimension', '$area_responsable')";
		$consulta = $mbd->prepare($sql);
		return $consulta->execute();
	}
	public function insertarReporteInfluencerGrupo($id_docente_grupo, $id_docente, $influencer_mensaje, $fecha, $hora, $periodo)
	{
		global $mbd;
		$sql = "INSERT INTO `influencer_reporte_grupo`(`id_docente_grupo`, `id_docente`, `influencer_mensaje`, `fecha`, `hora`, `periodo` ) VALUES('$id_docente_grupo', '$id_docente','$influencer_mensaje','$fecha','$hora','$periodo')";
		$consulta = $mbd->prepare($sql);
		return $consulta->execute();
	}
	...
	{
		$sql = "SELECT id_usuario, usuario_login, usuario_celular FROM usuario WHERE id_usuario= :id_usuario";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":id_usuario", $id_usuario);
		$consulta->execute();
	...
	public function listarReportesEstudiante($id_estudiante, $id_docente)
	{
		$sql = "SELECT * FROM `influencer_reporte` WHERE `id_estudiante` = :id_estudiante AND `id_docente` = :id_docente ORDER BY `id_influencer_reporte` DESC";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":id_estudiante", $id_estudiante);
		$consulta->bindParam(":id_docente", $id_docente);
	...
	{
		global $mbd;
		$sql = "SELECT CONCAT(`d`.`usuario_nombre`, ' ' ,`d`.`usuario_apellido`) AS `docente_nombre`, `ce`.`credencial_identificacion`, CONCAT(`ce`.`credencial_nombre`, ' ', `ce`.`credencial_nombre_2`, ' ', `ce`.`credencial_apellido`, ' ', `ce`.`credencial_apellido_2`) AS nombre_estudiante, `ce`.`credencial_login`, `e`.`fo_programa`, `ir`.`influencer_nivel_accion`, `ir`.`influencer_mensaje`, `ir`.`fecha`, `ir`.`hora`, `ir`.`id_influencer_reporte`  
	...
	public function listarReportesGrupo($id_docente_grupo, $id_docente)
	{
		$sql = "SELECT * FROM `influencer_reporte_grupo` WHERE `id_docente_grupo` = :id_docente_grupo AND `id_docente` = :id_docente ORDER BY `id_influencer_reporte` DESC";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":id_docente_grupo", $id_docente_grupo);
		$consulta->bindParam(":id_docente", $id_docente);
