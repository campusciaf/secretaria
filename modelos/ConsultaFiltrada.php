...
class ConsultaFiltrada
{
    public function listarMateria()
    {
        global $mbd;
        $sentencia = $mbd->prepare("SELECT `id_programa`,`nombre` FROM `programa_ac`");
        $sentencia->execute();
    ...
    public function consultaEstudiantes($programa,$jornada,$semestre){
        ...
        if ($sentencia->rowCount() > 0) {
            ...
            $data[]=array(
                ...
                '1'=> $datos['credencial_apellido'].' '.$datos['credencial_apellido_2'].' ' .$datos['credencial_nombre'].' '.$datos['credencial_nombre_2'].' ',
                '2'=> $programa['nombre'],
                '3'=> $registro['jornada_e'],
                '4'=> $registro['semestre_estudiante'],
                '5'=> $registro['grupo'],
                '6'=> '<img src="'.$foto.'" height="36px" width="36px" class="rounded-circle">',
                '7'=> ($datos['credencial_condicion'] == "1")? '<span class="badge badge-success">Activo</span>':'<span  class="badge badge-danger">Desactivado</span>'
            );
        }
        } else {
            $data[]=array(
