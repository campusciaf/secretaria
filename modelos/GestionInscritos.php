...
class GestionInscritos{
    //Implementar un método para listar los registros
    public function listarInscritos($roll){
        global $mbd;
        $tabla = ($roll == "Funcionario") ? "usuario" : $roll;
        if ($roll == 'Estudiante') {
            $sql = "SELECT `u`.`credencial_nombre` AS `usuario_nombre`, `u`.`credencial_nombre_2` AS `usuario_nombre_2`,`u`.`credencial_apellido` AS `usuario_apellido`, `u`.`credencial_apellido_2` AS `usuario_apellido_2`,`wec`.`nombre_curso`, `iec`.`create_dt`,`iec`.`estado_inscrito`, `iec`.`pago` FROM `inscritos_educacion_continuada` AS `iec` 
            ...
            WHERE `iec`.`roll` = 'Estudiante';";
        }else{
            $sql = "SELECT `u`.`usuario_nombre`, `u`.`usuario_nombre_2`,`u`.`usuario_apellido`, `u`.`usuario_apellido_2`,`wec`.`nombre_curso`, `iec`.`create_dt`,`iec`.`estado_inscrito`, `iec`.`pago`
