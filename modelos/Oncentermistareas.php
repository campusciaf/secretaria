...
class Consulta
{
    ...
    public function consulta_datos_dia($id,$tipo,$fecha_tarea,$fecha_hoy)
    {
        global $mbd;
        $sentencia = $mbd->prepare(" SELECT * FROM `on_interesados_tareas_programadas` WHERE `id_usuario` = :id AND motivo_tarea = :tipo AND fecha_registro>= :fecha_tarea AND fecha_programada= :fecha_hoy");
        $sentencia->bindParam(':id',$id);
        $sentencia->bindParam(':tipo',$tipo);
        $sentencia->bindParam(':fecha_tarea',$fecha_tarea);
    ...
    public function buscar($id,$tipo,$consulta,$periodo_campana)
    {
        global $mbd;
        $sentencia = $mbd->prepare(" SELECT * FROM `on_interesados_tareas_programadas` WHERE `id_usuario` = :id AND motivo_tarea $tipo $consulta AND periodo= :periodo_campana");
        $sentencia->bindParam(':id',$id);
