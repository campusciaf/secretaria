...
class Estudianteadmintarea
{
    ...
    public function fechashoymodal($fecha_programada)
    {
        global $mbd;
        $sentencia = $mbd->prepare(" SELECT * FROM `estudiante_tareas_programadas` WHERE `fecha_programada` = :fecha_programada and estado = 1 ");
        // echo $sentencia;
        // $sentencia->bindParam(':id',$id);
        // $sentencia->bindParam(':tipo',$tipo);
        $sentencia->bindParam(':fecha_programada',$fecha_programada);
    ...
    {
        global $mbd;
        $sentencia = $mbd->prepare(" UPDATE `estudiante_tareas_programadas` SET `estado`= 0, fecha_realizo = :fecha, hora_realizo = :hora WHERE `id_tarea` = :id ");
    ...
    public function trazabilidad_fecha($id_tarea,$fecha_anterior,$fecha_nueva)
    {
        ...
        $sentencia = $mbd->prepare(" INSERT INTO `egresados_trazabilidad_tareas`(`id_usuario`, `tipo`, `id_tarea`, `fecha_anterior`, `fecha_nueva`,`periodo`) VALUES (:id_usuario,1,:id_tarea,:fecha_anterior,:fecha_nueva,:periodo_actual) ");
    ...
    public function trazabilidad_asesor($id_tarea,$usuario_anterior,$usuario_nuevo)
    {
        ...
        $id_usuario = $_SESSION['id_usuario'];
        $periodo_actual = $_SESSION['periodo_actual'];
        $sentencia = $mbd->prepare(" INSERT INTO `egresados_trazabilidad_tareas`(`id_usuario`, `tipo`, `id_tarea`, `usuario_anterior`, `usuario_nuevo`,`periodo`) VALUES (:id_usuario,2,:id_tarea,:usuario_anterior,:usuario_nuevo,:periodo_actual) ");
        $sentencia->bindParam(":id_usuario",$id_usuario);
        $sentencia->bindParam(":id_tarea",$id_tarea);
