...
class CvReferenciasLaborales{
    ...
    public function cvinsertarReferencias($id_usuario_cv,$referencias_nombre,$referencias_profesion, $referencias_empresa, $referencias_telefono){
        $sql = "INSERT INTO `cv_referencias_laborales`(`id_usuario_cv`, `referencias_nombre`, `referencias_profesion`, `referencias_empresa`, `referencias_telefono`) VALUES(:id_usuario_cv, :referencias_nombre, :referencias_profesion, :referencias_empresa, :referencias_telefono);";
        global $mbd;
    ...
    public function cveditarReferencias($id_usuario_cv,$referencias_nombre,$referencias_profesion, $referencias_empresa, $referencias_telefono, $id_referencias){
        $sql = "UPDATE `cv_referencias_laborales` SET `id_usuario_cv`= :id_usuario_cv, `referencias_nombre`= :referencias_nombre, `referencias_profesion`= :referencias_profesion, `referencias_empresa`= :referencias_empresa, `referencias_telefono`= :referencias_telefono WHERE `id_referencias` =  :id_referencias";
        global $mbd;
        $consulta = $mbd->prepare($sql);
        $consulta->bindParam(":id_usuario_cv",$id_usuario_cv);
    ...
    public function cv_listarReferencias($documento){
        $sql = "SELECT `cv_referencias_laborales`.* FROM `cv_referencias_laborales` INNER JOIN `cv_usuario` ON `cv_referencias_laborales`.`id_usuario_cv` = `cv_usuario`.`id_usuario_cv` WHERE `cv_usuario`.`usuario_identificacion` = :documento";
        // $sql = "SELECT * FROM `cv_referencias_laborales` WHERE `id_usuario` = :id_usuario";
        global $mbd;
        $consulta = $mbd->prepare($sql);
    ...
    public function cvlistarReferenciasEspecifica($id_referencias){
        $sql = "SELECT * FROM `cv_referencias_laborales` WHERE `id_referencias` = :id_referencias";
        global $mbd;
        $consulta = $mbd->prepare($sql);
        $consulta->bindParam(":id_referencias",$id_referencias);
        $consulta->execute();
    ...
    public function cveliminarReferencias($id_referencias){
        $sql = "DELETE FROM `cv_referencias_laborales` WHERE `id_referencias` = :id_referencias";
        global $mbd;
        $consulta = $mbd->prepare($sql);
        $consulta->bindParam(":id_referencias",$id_referencias);
        $consulta->execute();
    ...
    public function cveditarCertificado($id_referencias, $nombre_img){
        $sql = "UPDATE `cv_referencias_laborales` SET `certificado_educacion` = :certificado_educacion WHERE `id_referencias` = :id_referencias";
        global $mbd;
        $consulta = $mbd->prepare($sql);
        $consulta->bindParam(":id_referencias",$id_referencias);
    ...
    {
        $sql = "SELECT COUNT(*) as total FROM cv_referencias_laborales WHERE id_usuario_cv = :id_usuario_cv";
        global $mbd;
        $stmt = $mbd->prepare($sql);
