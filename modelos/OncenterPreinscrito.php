...
Class OncenterPreinscrito
{
	...
	{	
		$sql="SELECT * FROM on_criterio_preinscrito WHERE estado=1";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->execute();
	...
	{	
		...
		if($nombre_criterio=="Preinscrito"){
		...
		if($nombre_criterio=="Mailing"){
			$sql="SELECT * FROM on_interesados WHERE estado='".$estado."' and mailing=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":periodo", $periodo);
			$consulta->execute();
		...
		if($nombre_criterio=="Formulario"){
			$sql="SELECT * FROM on_interesados WHERE estado='".$estado."' and formulario=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":periodo", $periodo);
			$consulta->execute();
		...
		if($nombre_criterio=="Inscripción"){
			$sql="SELECT * FROM on_interesados WHERE estado='".$estado."' and inscripcion=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":periodo", $periodo);
			$consulta->execute();
	...
	{	
		...
		if($nombre_criterio=="Mailing"){		
			$sql="SELECT * FROM on_interesados WHERE medio= :medio and estado='".$estado."' and mailing=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":medio", $medio);
			$consulta->bindParam(":periodo", $periodo);
		...
		if($nombre_criterio=="Formulario"){		
			$sql="SELECT * FROM on_interesados WHERE medio= :medio and estado='".$estado."' and formulario=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":medio", $medio);
			$consulta->bindParam(":periodo", $periodo);
		...
		if($nombre_criterio=="Inscripción"){		
			$sql="SELECT * FROM on_interesados WHERE medio= :medio and estado='".$estado."' and inscripcion=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":medio", $medio);
			$consulta->bindParam(":periodo", $periodo);
	...
	{	
		...
		if($nombre_criterio=="Mailing"){
			$sql="SELECT * FROM on_interesados WHERE fo_programa= :programa and jornada_e= :jornada and estado='".$estado."' and mailing=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":programa", $programa);
			$consulta->bindParam(":jornada", $jornada);
		...
		if($nombre_criterio=="Formulario"){
			$sql="SELECT * FROM on_interesados WHERE fo_programa= :programa and jornada_e= :jornada and estado='".$estado."' and formulario=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":programa", $programa);
			$consulta->bindParam(":jornada", $jornada);
		...
		if($nombre_criterio=="Inscripción"){
			$sql="SELECT * FROM on_interesados WHERE fo_programa= :programa and jornada_e= :jornada and estado='".$estado."' and inscripcion=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":programa", $programa);
			$consulta->bindParam(":jornada", $jornada);
	...
	{	
		...
		if($nombre_criterio=="Mailing"){
			$sql="SELECT * FROM on_interesados WHERE fo_programa= :programa and jornada_e= :jornada and medio= :medio and estado='".$estado."' and mailing=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":programa", $programa);
			$consulta->bindParam(":jornada", $jornada);
		...
		if($nombre_criterio=="Formulario"){
			$sql="SELECT * FROM on_interesados WHERE fo_programa= :programa and jornada_e= :jornada and medio= :medio and estado='".$estado."' and formulario=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":programa", $programa);
			$consulta->bindParam(":jornada", $jornada);
		...
		if($nombre_criterio=="Inscripción"){
			$sql="SELECT * FROM on_interesados WHERE fo_programa= :programa and jornada_e= :jornada and medio= :medio and estado='".$estado."' and inscripcion=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":programa", $programa);
			$consulta->bindParam(":jornada", $jornada);
	...
	{	
		...
		if($nombre_criterio=="Mailing"){
			$sql="SELECT * FROM on_interesados WHERE jornada_e= :jornada and estado='".$estado."' and mailing=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":jornada", $jornada);
			$consulta->bindParam(":periodo", $periodo);
		...
		if($nombre_criterio=="Formulario"){
			$sql="SELECT * FROM on_interesados WHERE jornada_e= :jornada and estado='".$estado."' and formulario=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":jornada", $jornada);
			$consulta->bindParam(":periodo", $periodo);
		...
		if($nombre_criterio=="Inscripción"){
			$sql="SELECT * FROM on_interesados WHERE jornada_e= :jornada and estado='".$estado."' and inscripcion=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":jornada", $jornada);
			$consulta->bindParam(":periodo", $periodo);
	...
	{	
		...
		if($nombre_criterio=="Mailing"){
			$sql="SELECT * FROM on_interesados WHERE jornada_e= :jornada and medio= :medio and estado='".$estado."' and mailing=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":jornada", $jornada);
			$consulta->bindParam(":medio", $medio);
		...
		if($nombre_criterio=="Formulario"){
			$sql="SELECT * FROM on_interesados WHERE jornada_e= :jornada and medio= :medio and estado='".$estado."' and formulario=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":jornada", $jornada);
			$consulta->bindParam(":medio", $medio);
		...
		if($nombre_criterio=="Inscripción"){
			$sql="SELECT * FROM on_interesados WHERE jornada_e= :jornada and medio= :medio and estado='".$estado."' and inscripcion=1 and periodo_campana= :periodo";
			global $mbd;
			$consulta = $mbd->prepare($sql);
			$consulta->bindParam(":jornada", $jornada);
			$consulta->bindParam(":medio", $medio);
	...
	{
		$sql="UPDATE on_interesados SET identificacion= :nuevodocumento, nombre_modalidad= :modalidad_campana, estado='Preinscrito', segui='1' WHERE id_estudiante= :id_estudiante_documento";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":nuevodocumento", $nuevodocumento);
...
{
    $sql = "SELECT * FROM on_seguimiento WHERE periodo_campana = :periodo AND mensaje_seguimiento = 'Preinscrito'";
    global $mbd;
    $consulta = $mbd->prepare($sql);
    $consulta->bindParam(":periodo", $periodo);
    $consulta->execute();
...
public function CantidadCasosPorCampañaUnaño($periodo)
{
	$sql = "SELECT * FROM on_seguimiento WHERE periodo_campana = :periodo AND mensaje_seguimiento = 'Preinscrito'";
	global $mbd;
	$consulta = $mbd->prepare($sql);
	...
	public function CantidadCasosPorCampañaAnterior($periodo)
	{
		$sql = "SELECT * FROM on_seguimiento WHERE periodo_campana = :periodo AND mensaje_seguimiento = 'Preinscrito'";
		global $mbd;
		$consulta = $mbd->prepare($sql);
	...
	public function temporadaanterior($periodo_actual)
	{
		$sql = "SELECT temporada FROM periodo WHERE periodo = :periodo_actual";
		global $mbd;
		$consulta = $mbd->prepare($sql);
	...
	public function periodoactualtemporada($temporada)
	{
		$sql = "SELECT * FROM periodo WHERE temporada = :temporada";
		// echo $sql;
		global $mbd;
		$consulta = $mbd->prepare($sql);
	...
	public function TraerCorreoInstitucional($credencial_identificacion)
	{
