...
class FactorDocente
{
	...
	public function selectPeriodo(){	
		...
		//Implementar un método para listar los programas en un select
	public function selectNivelFormacion(){	
...
public function total($periodo,$escuela,$nivel,$jornada,$semestre,$programa,$contrato,$nivel_formacion){

		$sql="SELECT * FROM contrato_docente cd INNER JOIN docente doc ON doc.usuario_identificacion=cd.documento_docente WHERE periodo= :periodo  $escuela $nivel $jornada $semestre $programa $contrato $nivel_formacion";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":periodo", $periodo);
		$consulta->execute();
