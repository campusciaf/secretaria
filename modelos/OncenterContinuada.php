...
class OncenterContinuada
{
	...
	public function verificarcuenta($identificacion)
    {
        global $mbd;
        $sentencia = $mbd->prepare(" SELECT * FROM educacion_continuada_estudiantes WHERE continuada_identificacion = :identificacion");
        $sentencia->bindParam(":identificacion", $identificacion);
		$sentencia->execute();
        return $sentencia->fetchAll(PDO::FETCH_ASSOC);
    }
    public function consultaestudiantes($val)
    {
        global $mbd;
        $sentencia = $mbd->prepare(" SELECT * FROM credencial_estudiante ce INNER JOIN estudiantes_datos_personales edp ON ce.id_credencial=edp.id_credencial WHERE ce.credencial_identificacion = $val ");
        $sentencia->execute();
        return $sentencia->fetchAll(PDO::FETCH_ASSOC);
    }
    public function consultadocente($val)
    {
        global $mbd;
        $sentencia = $mbd->prepare(" SELECT * FROM `docente` WHERE usuario_identificacion = $val ");
        $sentencia->execute();
        return $sentencia->fetchAll(PDO::FETCH_ASSOC);
    }

    public function consultafuncionarios($val)
    {
        global $mbd;
        $sentencia = $mbd->prepare(" SELECT * FROM `usuario` WHERE usuario_identificacion = $val ");
        $sentencia->execute();
        return $sentencia->fetchAll(PDO::FETCH_ASSOC);
    }
	public function consultacontinuada($val)
    {
        global $mbd;
        $sentencia = $mbd->prepare(" SELECT * FROM `educacion_continuada_estudiantes` WHERE continuada_identificacion = $val ");
        $sentencia->execute();
        return $sentencia->fetchAll(PDO::FETCH_ASSOC);
    }

    public function consultacursos($val)
    {
        global $mbd;
        $sentencia = $mbd->prepare(" SELECT * FROM educacion_continuada_interesados edi INNER JOIN web_educacion_continuada wec ON edi.id_curso=wec.id_curso WHERE edi.identificacion = $val");
        $sentencia->execute();
        return $sentencia->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    {
        global $mbd;
        $sentencia = $mbd->prepare(" SELECT * FROM `educacion_continuada_estudiantes` WHERE credencial_identificacion = $val ");
        $sentencia->execute();
        return $sentencia->fetchAll(PDO::FETCH_ASSOC);
    }
	...
	{	
		$sql="SELECT * FROM web_educacion_continuada WHERE estado_educacion=1";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->execute();
	...
	public function insertarClienteNc($identificacion,$id_curso,$periodo_ingreso,$fecha,$hora,$medio,$conocio,$contacto,$estado,$periodo_campana,$id_usuario)
	{
		$sql="INSERT INTO  educacion_continuada_interesados(identificacion,id_curso,periodo_ingreso,fecha_ingreso,hora_ingreso,medio,conocio,contacto,estado_interesado,periodo_campana,id_usuario) VALUES('$identificacion','$id_curso','$periodo_ingreso','$fecha','$hora','$medio','$conocio','$contacto','$estado','$periodo_campana','$id_usuario')";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->execute();
	...
	public function mostrarDatosEstudiante($identificacion)
	{
	...
	//Implementamos un método para insertar un nuevo estudiante en continuada
	public function insertarClienteEstudiante($usuario_identificacion_mc,$usuario_nombre_mc,$usuario_nombre_2_mc,$usuario_apellido_mc,$usuario_apellido_2_mc,$usuario_login,$continuada_clave,$id_usuario,$fecha)
	{
		$sql="INSERT INTO educacion_continuada_estudiantes(continuada_nombre,continuada_nombre_2,continuada_apellido,continuada_apellido_2,continuada_identificacion,continuada_login,continuada_clave,continuada_usuario,continuada_fecha)
		VALUES ('$usuario_nombre_mc','$usuario_nombre_2_mc','$usuario_apellido_mc','$usuario_apellido_2_mc','$usuario_identificacion_mc','$usuario_login','$continuada_clave','$id_usuario','$fecha')";
		...
		//Implementamos un método para insertar un nuevo estudiante en continuada
		public function insertarClienteEstudianteDatos($id_credencial_continuada,$continuada_datos_correo,$continuada_datos_telefono)
		{
			$sql="INSERT INTO educacion_continuada_estudiantes_datos(id_credencial_continuada,continuada_datos_correo,continuada_datos_telefono)
			VALUES ('$id_credencial_continuada','$continuada_datos_correo','$continuada_datos_telefono')";
	...
	//Implementar un método para mostrar los datos de un registro a modificar
	public function mostrarDatosDocente($identificacion)
	{
		$sql = "SELECT * FROM docente WHERE usuario_identificacion= :identificacion";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":identificacion", $identificacion);
		$consulta->execute();
	...
	//Implementar un método para mostrar los datos de un registro a modificar
	public function mostrarDatosFuncionario($identificacion)
	{
		$sql = "SELECT * FROM usuario WHERE usuario_identificacion= :identificacion";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":identificacion", $identificacion);
		$consulta->execute();
