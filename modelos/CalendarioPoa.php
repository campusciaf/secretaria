...
Class CalendarioPoa
{
	...
	// Función para cargar los eventos por usuario en la sesión activa 
	public function mostrarEventosIndivual($id_usuario){

		// SELECT `nombre_accion` FROM `sac_meta` INNER JOIN `sac_accion` ON `sac_accion`.`id_meta` = `sac_meta`.`id_meta` WHERE `sac_meta`.`id_proyecto` = :id_proyecto
		//Mostrar proyectos por id usuario
		// SELECT `sac_meta`.`id_meta` as id, `sac_meta`.`meta_nombre` as title, `sac_meta`.`meta_fecha` as start, `sac_meta`.`id_proyecto`,`sac_meta`.`meta_responsable`, `sac_accion`.`nombre_accion`, `sac_meta`.`anio_eje` FROM `sac_meta` INNER JOIN `sac_accion` ON `sac_accion`.`id_meta` = `sac_meta`.`id_meta` INNER JOIN `usuario` ON `sac_meta`.`id_meta` = `sac_accion`.`id_meta` WHERE `usuario`.`id_usuario` = :id_usuario
