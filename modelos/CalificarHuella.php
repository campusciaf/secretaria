...
class CalificarHuella
{
	...
	{
		global $mbd;
		$sql = "SELECT `credencial_estudiante`.`id_credencial`,`credencial_estudiante`.`credencial_nombre`, `credencial_estudiante`.`credencial_nombre_2`, `credencial_estudiante`.`credencial_apellido`,`credencial_estudiante`.`credencial_apellido_2`,`credencial_estudiante`.`credencial_login`, `estudiantes_datos_personales`.`celular` FROM `credencial_estudiante` INNER JOIN `estudiantes_datos_personales` ON `credencial_estudiante`.`id_credencial` = `estudiantes_datos_personales`.`id_credencial` WHERE `credencial_estudiante`.`id_credencial` = :id_credencial";
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":id_credencial", $id_credencial);
		$consulta->execute();
	...
	public function trazabilidadhuella($i, $tl, $mate, $nota)
	{
		...
		$nota_anterior = isset($registro[$tl]) ? $registro[$tl] : '0';
