...
class DashboardDoc
{
    ...
    public function fechaesp($date){
        ...
        // Asegura que $year, $month, y $day son enteros
        $year = intval($dia[0]);
        $month = intval($dia[1]);
        $day = intval($dia[2]);

        $dias = array("domingo", "lunes", "martes", "miércoles", "jueves", "viernes", "sábado");
        $tomadia = $dias[date("w", mktime(0, 0, 0,
            $month,
            $day,
            $year
        ))];
    ...
    public function DocenteIngresos($fecha, $id_usuario) {
        $sql = "SELECT * FROM ingresos_campus WHERE roll = 'Docente' AND id_usuario = :id_usuario AND fecha = :fecha";
        global $mbd; 
        $sql_mostrar = $mbd->prepare($sql);
        $sql_mostrar->bindParam(":id_usuario", $id_usuario);
        $sql_mostrar->bindParam(":fecha", $fecha);
        $sql_mostrar->execute();
        $result = $sql_mostrar->fetchAll(); // Fetching all results as associative array
        return $result;
    }
