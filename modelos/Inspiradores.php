...
Class Inspiradores
{
	...
	public function listar2($periodo){	
		$sql="SELECT * FROM caracterizacion_data cd INNER JOIN credencial_estudiante ce ON cd.id_credencial=ce.id_credencial  INNER JOIN caracterizacion_inspiradores ci ON cd.id_credencial = ci.id_credencial WHERE cd.periodo= :periodo";
		global $mbd;
	...
	public function listar($periodo){	
		$sql="SELECT * FROM estudiantes est INNER JOIN credencial_estudiante ce ON est.id_credencial=ce.id_credencial INNER JOIN estudiantes_datos_personales edp ON est.id_credencial = edp.id_credencial INNER JOIN caracterizacion_inspiradores ci ON est.id_credencial = ci.id_credencial WHERE est.periodo= :periodo";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->bindParam(":periodo", $periodo);
		$consulta->execute();
	...
	{
		$sql="SELECT * FROM variables WHERE id_categoria=2";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->execute();
		$resultado = $consulta->fetchAll();
