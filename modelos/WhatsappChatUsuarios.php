...
session_start();
class WhatsappChatUsuarios
{
    function listarChats($valor_buscar, $id_usuario){
        global $mbd;
        ...
        if (!empty($valor_buscar)) {
            ...
            $sql .= " WHERE `wes`.`estado` = 'activo' AND `wes`.`id_usuario` = :id_usuario";
        }
        // Agregar el orden
    ...
    function CantidadNoMostrados($id_usuario){
        global $mbd;
    ...
    function actualizarMostrados($id_usuario){
        global $mbd;
