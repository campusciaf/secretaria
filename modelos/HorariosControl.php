...
Class HorariosControl
{
    ...
    public function listar($id_escuela,$periodo_actual)
    {	
    
        $sql="SELECT 
        dg.id_programa,dg.periodo,dg.id_docente,dg.ciclo As ciclo_doc_grupos,dg.jornada AS doc_grupo_jornada,dg.semestre AS doc_grupo_semestre,
        dg.grupo AS doc_grupo_grupo,dg.dia,dg.hora,dg.hasta,dg.diferencia,dg.salon,dg.ciclo,dg.fusion_salon,dg.fusion_docente,
        pa.id_programa,pa.escuela,
        mc.id,mc.nombre AS materia_nombre,
        doc.id_usuario,doc.usuario_nombre,doc.usuario_apellido
        FROM docente_grupos dg 
        INNER JOIN programa_ac pa ON dg.id_programa=pa.id_programa 
        INNER JOIN materias_ciafi mc ON dg.id_materia=mc.id 
        INNER JOIN docente doc ON dg.id_docente=doc.id_usuario 
        WHERE pa.escuela= :id_escuela and dg.periodo= :periodo_actual ";
        global $mbd;
        $consulta = $mbd->prepare($sql);
    ...
    public function traerestudiantes($ciclo,$nombre_materia,$jornada,$periodo,$semestre,$id_programa,$grupo){
     ...
     {	
       ...
       {	
          ...
          {	
              ...
              $sql="SELECT * FROM estudiantes WHERE id_programa_ac= :id_programa and periodo = :periodo_actual and estado='1' and periodo_activo= :periodo_actual and admisiones='no' and hoologado='1'";
              global $mbd;
              $consulta = $mbd->prepare($sql);
              $consulta->bindParam(":id_programa", $id_programa);
              $consulta->bindParam(":periodo_actual", $periodo_actual);
