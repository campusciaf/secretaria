...
class PanelEstudiante
{
    ...
    {
        $sql = "SELECT * FROM faltas WHERE id_estudiante= :id_usuario and fecha_falta= :fecha";
    ...
    public function consulta_materiasparanotas($id_estudiante, $ciclo)
    {
        // metodo que trae las materias matriculadas
        global $mbd;
        $periodo = $_SESSION['periodo_actual'];
    ...
    // public function consulta_clasedeldia($id_estudiante,$ciclo){
    //     // metodo que trae las materias matriculadas
    //     global $mbd;
    //     $periodo = $_SESSION['periodo_actual'];
    //     // $fecha = date('Y-m-d');
    ...
    {
        $sql_buscar = "SELECT * FROM `calendario` WHERE `fecha_inicio` LIKE '%$fecha_actual%' ORDER BY DAY(`fecha_inicio`)";
        // echo $sql_buscar;
        global $mbd;
    ...
    public function GuardarmonitoreandoEstudiante($id_usuario_estudiante, $fecha, $p1, $p2, $p3, $p4, $p5, $p6, $p7, $p8, $p9, $p10, $p11, $p12, $p13, $p14, $p15) {
        $sql = "INSERT INTO `monitoreando_estudiantes`(`id_usuario_estudiante`, `fecha`, `p1`, `p2`, `p3`, `p4`, `p5`, `p6`, `p7`, `p8`, `p9`, `p10`, `p11`, `p12`, `p13`, `p14`, `p15`) 
                VALUES (:id_usuario_estudiante, :fecha, :p1, :p2, :p3, :p4, :p5, :p6, :p7, :p8, :p9, :p10, :p11, :p12, :p13, :p14, :p15);";
        global $mbd;
        $consulta = $mbd->prepare($sql);
        $consulta->bindParam(":id_usuario_estudiante", $id_usuario_estudiante);
        $consulta->bindParam(":fecha", $fecha);
    ...
    public function verificarRegistromonitoreandoestudiante($id_usuario_estudiante)
    {
        global $mbd;
        $sql = "SELECT * FROM `monitoreando_estudiantes` WHERE `id_usuario_estudiante` = :id_usuario_estudiante ";
        $sentencia = $mbd->prepare($sql);
        $sentencia->bindParam(":id_usuario_estudiante", $id_usuario_estudiante);
        $sentencia->execute();
        return $sentencia->fetch(PDO::FETCH_ASSOC);
    }
    

    public function estadoModalmonitoreandoEstudiante($id_usuario_estudiante)
    {
        $sql = "SELECT * FROM `monitoreando_estudiantes` WHERE  `id_usuario_estudiante`= :id_usuario_estudiante";
        global $mbd;
        $consulta = $mbd->prepare($sql);
	...
	public function validarpuntos($evento,$fecha,$id_usuario){	
		...
		$sql=" SELECT * FROM puntos WHERE id_credencial= :id_usuario and punto_nombre= :evento and punto_fecha= :fecha";
		global $mbd;
		$consulta = $mbd->prepare($sql);
