...
Class SacCrearCompromiso {
	...
	public function sac_insertar($id_usuario_compromiso,$nombre_compromiso,$fecha_compromiso,$compromiso_val_admin,$compromiso_val_usuario,$resposable_compromiso,$compromiso_anio)
	{
		$sql="INSERT INTO sac_compromiso (id_usuario_compromiso,nombre_compromiso,fecha_compromiso,compromiso_val_admin,compromiso_val_usuario,resposable_compromiso,compromiso_anio)
		VALUES ('$id_usuario_compromiso','$nombre_compromiso','$fecha_compromiso','$compromiso_val_admin','$compromiso_val_usuario','$resposable_compromiso','$compromiso_anio')";
		global $mbd;
	...
	public function sac_editar($id_compromiso,$id_usuario_compromiso,$nombre_compromiso,$fecha_compromiso,$compromiso_val_admin,$compromiso_val_usuario,$resposable_compromiso,$compromiso_anio)
	{
		$sql="UPDATE sac_compromiso SET id_usuario_compromiso='$id_usuario_compromiso', nombre_compromiso	='$nombre_compromiso', fecha_compromiso='$fecha_compromiso',compromiso_val_admin='$compromiso_val_admin', resposable_compromiso='$resposable_compromiso', compromiso_anio='$compromiso_anio', compromiso_val_usuario='$compromiso_val_usuario' WHERE id_compromiso='$id_compromiso'";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		return $consulta->execute();
	}
	//Implementamos un método para insertar registros
	public function sac_insertarmeta($id_compromiso_meta,$id_eje_meta,$nombre_meta,$meta_val_admin,$meta_val_usuario,$meta_responsable,$meta_anio,$condicion_institucional_meta,$condicion_programa)
	{
		$sql="INSERT INTO sac_meta (id_compromiso_meta,id_eje_meta,nombre_meta,meta_anio,meta_val_admin,fecha_val_admin,meta_val_usuario,fecha_val_usuario,meta_responsable,meta_anio)
		VALUES ('$id_compromiso_meta','$id_eje_meta','$nombre_meta','$meta_anio','$meta_val_admin','0000-00-00','$meta_val_usuario','0000-00-00','$meta_responsable','$meta_anio')";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->execute();
		...
		{
			$sql_detalle = "INSERT INTO meta_con_ins(id_compromiso,id_meta, condicion_institucional_meta) VALUES('$id_compromiso_meta','$idusuarionew', '$condicion_institucional_meta[$num_elementos]')";
	...
	public function editarmeta($id_meta,$id_compromiso_meta,$id_eje,$meta_nombre,$meta_fecha,$meta_val_admin,$meta_val_usuario,$meta_valida,$meta_periodo,$corresponsabilidad,$condicion_institucional,$condicion_programa)
	{
		...
		//Eliminamos todos las condicones de programa asignados para volverlos a registrar
		...
		//Eliminamos todos las condicones de programa asignados para volverlos a registrar
		$sqldel3="DELETE FROM meta_con_pro WHERE id_meta= :id_meta";
		//ejecutarConsulta($sqldel);
	...
	public function mostrarMeta($id_compromiso_meta,$meta_anio)
	{
		$sql="SELECT * FROM sac_meta WHERE id_compromiso_meta= :id_compromiso_meta and meta_anio='$meta_anio'";
		global $mbd;
		$consulta = $mbd->prepare($sql);
	...
	{	
		...
		//Eliminamos la meta
		$sql1="DELETE FROM sac_meta WHERE id_compromiso= :id_compromiso";
	...
	//Implementar un método para listar los compromisos
	public function saclistarCompromiso($compromiso_periodo)
	{	
		//SELECT * FROM sac_compromiso WHERE id_usuario='".$_SESSION['id_usuario']."' and compromiso_periodo='$compromiso_periodo'
		$sql="SELECT * FROM sac_compromiso WHERE id_usuario_compromiso='".$_SESSION['id_usuario']."' and compromiso_periodo='$compromiso_periodo'";
		global $mbd;
		$consulta = $mbd->prepare($sql);
		$consulta->execute();
		$resultado = $consulta->fetchAll();
