...
class WhatsappGestionCliente{
    function listarChats($valor_buscar, $estado_chat){
        ...
        if (!empty($valor_buscar)) {
            ...
            OR `ce`.`credencial_identificacion` LIKE :valor_buscar
            OR `ce`.`credencial_apellido` LIKE :valor_buscar
            OR `ce`.`credencial_nombre_2` LIKE :valor_buscar
            OR `ce`.`credencial_nombre` LIKE :valor_buscar) ";
        } else {
            $sql .= " 1 ";
        }
    ...
    function CantidadNoMostrados(){
        ...
        global $mbd;
        $sentencia = $mbd->prepare("SELECT COUNT(*) AS `total_sin_mostrar` FROM `whatsapp_registros` `wr` WHERE `wr`.`mostrado` = 0;");
        $sentencia->execute();
    ...
    public function EstadoSeguimiento($numero_celular){
        $numero_celular = substr($numero_celular, 2);
        global $mbd;
        $sentencia = $mbd->prepare("SELECT `mensaje_bot` FROM `whatsapp_registros` WHERE `numero_whatsapp` LIKE :numero_celular;");
        $sentencia->bindParam(":numero_celular", $numero_celular);
        $sentencia->execute();
        $registros = $sentencia->fetch(PDO::FETCH_ASSOC);
        return isset($registros["mensaje_bot"])? $registros["mensaje_bot"]:false;
    }
}
?>