...
class ConsultaPromedio
{
    ...
    public function consultaPromedio($programa, $periodo, $jornada)
    {
        $ciclo = self::consultaMateria($programa);
        $materia = "materias" . $ciclo['ciclo'];
        $data = array();
        // Agregamos filtro por jornada
        $sql = "SELECT DISTINCT id_estudiante FROM $materia  WHERE periodo = :periodo AND programa = :programa AND jornada = :jornada";
        // echo $sql;
        global $mbd;
